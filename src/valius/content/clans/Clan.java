...
public class Clan {
	...
	public void sendChat(Player paramClient, String paramString) {
		...
		for (Player player : PlayerHandler.getPlayersForNames(activeMembers)) {
			...
			if(player.getIgnores().has(paramClient.getNameAsLong()))
				continue;
			player.sendMessage("/@bla@[@blu@" + getTitle() + "@bla@] " + icon + "@bla@" + Misc.optimizeText(paramClient.playerName) + ": @dre@"
	...
	public void handleLootShare(Player c, int item, int amount) {
		...
		if (c.getShops().getItemShopValue(item, 1, c.getItems().getItemSlot(item)) > 1) {
			sendLootShareMessage("[Loot Bot] " + c.playerName + " has received " + amount + "x " + valius.model.items.ItemUtility.getItemName(item) + ".");
		}
	}
