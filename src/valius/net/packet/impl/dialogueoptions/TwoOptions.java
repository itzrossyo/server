...
import valius.content.barrows.RoomLocation;
...
public class TwoOptions {
	...
	public static void handleOption1(Player c) {
		...
		case 10000:
			...
			if(raidInstance != null) {
				c.sendMessage("@blu@You are now leaving the raid...");
				raidInstance.leaveGame(c);
			} else {
				c.sendMessage("@red@You need to be in a raid to do this...");
			}
		...
		case 903:
			...
			break;
		case 23682765:
			c.getDH().sendDialogues(23682766, 8369);
			break;
			
		case 10001:
			c.getPA().spellTeleport(2364, 4949, 0, true);
			c.sendMessage("You teleport to the Donator Thermonuclear smoke devil dungeon.");
		...
		case 4008:
			...
			if (c.getItems().playerHasItem(2699, 1)) {
				...
				other.amDonated += 100;
				c.amDonated -= 100;
				other.sendMessage("$100 has been added to your total amount donated.");
				c.sendMessage("$100 has been removed from your amount donated and given to the other player.");
				c.updateRank();
				other.updateRank();
				c.getPA().closeAllWindows();
			}
		...
		case 4009:
			...
			if (c.getItems().playerHasItem(2700, 1)) {
				...
				other.amDonated += 5;
				c.amDonated -= 5;
				other.sendMessage("$5 has been added to your total amount donated.");
				c.sendMessage("$5 has been removed from your amount donated and given to the other player.");
				c.updateRank();
				other.updateRank();
				c.getPA().closeAllWindows();
			}
		...
		case 4005:
			...
			break;
		case 811:
			Item item = new Item(c.unfPotHerb);
		...
		if (c.dialogueAction == 4004) {
			...
			if (c.getItems().playerHasItem(2701, 1)) {
				...
				c.getItems().addItemUnderAnyCircumstance(33508, 1);
		...
		if (c.dialogueAction == 2109) {
			if (c.getX() >= 2438 && c.getX() <= 2439 && c.getY() >= 5168 && c.getY() <= 5169) {
		...
		if (c.dialogueAction == 6) {
			...
			// @gre@"+World.getWorld().getNpcHandler().getNpcListName(c.slayerTask)+
			// " ", 7383);
			// c.getPA().closeAllWindows();
		}
		// if (c.dialogueAction == 29) {
	...
	public static void handleOption2(Player c) {
		...
		case 10001:
			if (c.getRights().isOrInherits(Right.SAPPHIRE) && c.getItems().playerHasItem(995, 5_000_000)) {
				c.getItems().deleteItem(995, 5_000_000);
			} else if (c.getRights().isOrInherits(Right.EMERALD) && c.getItems().playerHasItem(995, 4_000_000)) {
				c.getItems().deleteItem(995, 4_000_000);
			} else if (c.getRights().isOrInherits(Right.RUBY) && c.getItems().playerHasItem(995, 3_000_000)) {
				c.getItems().deleteItem(995, 3_000_000);
			} else if (c.getRights().isOrInherits(Right.DIAMOND) && c.getItems().playerHasItem(995, 2_000_000)) {
				c.getItems().deleteItem(995, 2_000_000);
			} else if (c.getRights().isOrInherits(Right.DRAGONSTONE) && c.getItems().playerHasItem(995, 1_000_000)) {
				c.getItems().deleteItem(995, 1_000_000);
			} else if (c.getRights().isOrInherits(Right.ONYX) && c.getItems().playerHasItem(995, 500_000)) {
				c.getItems().deleteItem(995, 500_000);
			} else {
				if (c.getRights().isOrInherits(Right.SAPPHIRE)) {
				c.sendMessage("You need 5,000,000 coins to fight this boss!");
				break;
				} else if (c.getRights().isOrInherits(Right.EMERALD)) {
					c.sendMessage("You need 4,000,000 coins to fight this boss!");
					break;
				} else if (c.getRights().isOrInherits(Right.RUBY)) {
					c.sendMessage("You need 3,000,000 coins to fight this boss!");
					break;
				} else if (c.getRights().isOrInherits(Right.DIAMOND)) {
					c.sendMessage("You need 2,000,000 coins to fight this boss!");
					break;
				} else if (c.getRights().isOrInherits(Right.DRAGONSTONE)) {
					c.sendMessage("You need 1,000,000 coins to fight this boss!");
					break;
				} else if (c.getRights().isOrInherits(Right.ONYX)) {
					c.sendMessage("You need 500,000 coins to fight this boss!");
					break;
				}
			}
			c.getPA().spellTeleport(2364, 4949, 0, true);
			c.sendMessage("You teleport to Nightmares Dungeon.");
			c.getDH().sendDialogues(10001, 0);
			break;
		...
		case 705:
			...
			break;
		case 23682765:
			c.getDH().sendDialogues(23682767, 8369);
			break;
		case 65:
			c.getDiaryManager().getVarrockDiary().progress(VarrockDiaryEntry.GRAND_TREE_TELEPORT);
			switch (c.getDH().tree) {
