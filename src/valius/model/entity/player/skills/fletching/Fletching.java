...
public class Fletching {
	...
	public void fletchLog(FletchableLog fletchable, int amount) {
		...
		if (player.getSkills().getLevel(Skill.FLETCHING) < fletchable.getLevel()) {
		...
		World.getWorld().getEventHandler().submit(new FletchLogEvent(player, 3, fletchable, amount));
	}
	...
	public void fletchGem(int use, int used) {
		...
		gem.ifPresent(g -> {
			...
			if (player.getSkills().getLevel(Skill.FLETCHING) < g.getLevel()) {
	...
	public void fletchUnfinishedBolt(int boltId) {
		...
		bolt.ifPresent(b -> {
			...
			if (player.getSkills().getLevel(Skill.FLETCHING) < b.getLevel()) {
			...
			player.getPA().addSkillXP(b.getExperience(), Skill.FLETCHING.getId(), true);
			if (Misc.random(50) == 0) {
				int sPoints = Misc.random(1, 5);
                player.skillPoints += sPoints;
                player.sendMessage("@pur@You receive " + sPoints + " Skill Points.");
	...
	public void fletchUnstrung(int bowId) {
		...
		bow.ifPresent(b -> {
			player.getSkilling().stop();
			if (player.getSkills().getLevel(Skill.FLETCHING) < b.getLevelRequired()) {
				player.sendMessage("You need a fletching level of " + b.getLevelRequired() + " to do this.");
				player.getPA().removeAllWindows();
				return;
			}
			player.getSkilling().setSkill(Skill.FLETCHING);
			World.getWorld().getEventHandler().submit(new StringBowEvent(b, player, 3));
			if (Misc.random(40) == 0) {
				int sPoints = Misc.random(1, 5);
                player.skillPoints += sPoints;
                player.sendMessage("@pur@You receive " + sPoints + " Skill Points.");
	...
	public void fletchUnstrungCross(int crossbowId) {
		...
		crossbow.ifPresent(b -> {
			...
			player.getSkilling().setSkill(Skill.FLETCHING);
			World.getWorld().getEventHandler().submit(new StringCrossbowEvent(b, player, 3));
			if (Misc.random(40) == 0) {
				int sPoints = Misc.random(1, 5);
                player.skillPoints += sPoints;
                player.sendMessage("@pur@You receive " + sPoints + " Skill Points.");
	...
	public void fletchArrow(int arrowId) {
		...
		arrow.ifPresent(a -> {
			player.getSkilling().stop();
			if (player.getSkills().getLevel(Skill.FLETCHING) < a.getLevelRequired()) {
			...
			player.getPA().addSkillXP((int) a.getExperience(), Skill.FLETCHING.getId(), true);
			if (Misc.random(50) == 0) {
				int sPoints = Misc.random(1, 5);
                player.skillPoints += sPoints;
                player.sendMessage("@pur@You receive " + sPoints + " Skill Points.");
	...
	public void fletchDart(int dartId) {
		...
		dart.ifPresent(d -> {
			player.getSkilling().stop();
			if (player.getSkills().getLevel(Skill.FLETCHING) < d.getLevelRequired()) {
			...
			player.getPA().addSkillXP((int) (10 * d.getExperience()), Skill.FLETCHING.getId(), true);
			if (Misc.random(50) == 0) {
				int sPoints = Misc.random(1, 5);
                player.skillPoints += sPoints;
                player.sendMessage("@pur@You receive " + sPoints + " Skill Points.");
