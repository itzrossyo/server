	...
	public void accept(Player player, Player recipient, int stageId) {
		...
		case MultiplayerSessionStage.OFFER_ITEMS:
			...
			for (Player p : players) {
				Item overlap = getOverlappedItem(p);
				if (overlap != null) {
...
/*				PlayerLogging.write(LogType.TRADE_LOG, player,
	...
	@Override
	public boolean itemAddable(Player player, Item item) {
		if (item.getId() == 12650 || item.getId() == 12649 || item.getId() == 12651 || item.getId() == 12652 || item.getId() == 20851 || item.getId() == 12644 || item.getId() == 12645 || item.getId() == 12643 || item.getId() == 11995 || item.getId() == 15568 || item.getId() == 12653 || item.getId() == 12655 || item.getId() == 13178 || item.getId() == 12646 || item.getId() == 13179 || item.getId() == 13177 || item.getId() == 12921 || item.getId() == 13181 || item.getId() == 12816 || item.getId() == 12647) {
	...
	@Override
	public boolean itemRemovable(Player player, Item item) {
	...
	public void give() {
		...
		for (Player player : items.keySet()) {
			...
			try {
				String tradedItems = items.get(player).stream().map(item -> ("\t\t" + item.getId() + ":" + item.getAmount())).collect(Collectors.joining("\n"));
				PlayerLogging.write(LogType.TRADE_LOG, player, player.playerName + " gave " + getOther(player).playerName + " {\n" + tradedItems + "\n}");
			} catch(Exception ex) {
				ex.printStackTrace();
			}
		}
