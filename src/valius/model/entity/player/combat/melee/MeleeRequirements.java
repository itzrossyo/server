...
import valius.model.holiday.christmas.ThrowingSnowball;
...
public class MeleeRequirements {
	...
	public static int getRequiredDistance(Player c) {
		if (c.followId > 0 && c.freezeTimer <= 0 && !c.getMovementQueue().isMoving())
			return 2;
		else if (c.followId > 0 && c.freezeTimer <= 0 && c.getMovementQueue().isMoving()) {
			return 4;
		} else {
			return 1;
		}
