...
public abstract class ItemCombination {
	 ...
	 */
	public ItemCombination(Item outcome, Optional<List<Item>> revertedItems, Item... items) {
	...
	public boolean isCombinable(Player player) {
		Optional<Item> unavailableItem = items.stream().filter(i -> !player.getItems().playerHasItem(i.getId(), i.getAmount())).findFirst();
	 ...
	 */
	public boolean itemsMatch(Item item1, Item item2) {
		return items.stream().filter(itemValuesMatch(item1).or(itemValuesMatch(item2))).count() == 2;
	}

	private static final Predicate<Item> itemValuesMatch(Item item) {
