package valius.model.entity.player.skills.hunter.trap.impl;
...
//		npc = null;
	...
	public boolean canCatch(NPC npc) {
		...
		if(player.getSkills().getLevel(Skill.HUNTER) < data.get().requirement) {//(player.getSkills().getLevel(player.playerHunter] < data.get().requirement)FIXME
		...
		event = Optional.of(new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				if(npc.getX() == getObject().getX() && npc.getY() == getObject().getY()) {
					...
					int count = random.inclusive(100);
...
//				return;
//			}
			if(this.getObject().getHeight() == npc.getHeight() && Math.abs(this.getObject().getX() - npc.getX()) <= DISTANCE_PORT && Math.abs(this.getObject().getY() - npc.getY()) <= DISTANCE_PORT) {
	...
	public void setState(TrapState state) {
		...
		if(state.equals(TrapState.FALLEN)) {
			...
			World.getWorld().getGlobalObjects().add(this.getObject());
		}
	...
	private enum BoxTrapData {
		...
		BLACK_CHINCHOMPA(2912, 73, 335, new Item (11959));
