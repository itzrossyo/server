...
public class AchievementHandler {
    ...
    public void drawInterface(int tier) {
        ...
        for (Achievement achievement : Achievement.ACHIEVEMENTS) {
            if (achievement.getTier().ordinal() == tier) {
               ...
               // player.getPA().sendFrame126(achievement.name().replaceAll("_", " "), count);
                player.getPA().sendFrame126(Misc.capitalizeJustFirst(achievement.name().toLowerCase().replaceAll("_", " ")), count);
                count++;
            }
        }
    ...
    public void viewAchievement(int achievement, int tier) {
        ...
        for (Achievement ach : Achievement.ACHIEVEMENTS) {
            if (ach.getTier().ordinal() == tier) {
                ...
                if (count == toView) {
                    int descLines;
                    player.getPA().sendFrame126(Misc.capitalizeJustFirst(ach.name().toLowerCase().replaceAll("_", " ")), 36501);//Title
    ...
    public void kill(NPC npc) {
        String name = World.getWorld().getNpcHandler().getNpcListName(npc.npcType);
        if (name.length() <= 0) {
            return;
        } else {
    ...
    public void claimCape() {
        if (!hasCompletedAll()) {
            player.getDH().sendDialogues(1, 5527); //5527
            return;
        }
        if (!player.getItems().playerHasItem(995, 99_000)) {
            player.getDH().sendDialogues(2, 5527); //5527
            return;
        }
