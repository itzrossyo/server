...
public enum SkillcapePerks {
		...
		MASTER_AGILITY(new int[][] { { 33033 } }),
		MASTER_ATTACK(new int[][] { { 33034 } }),
		MASTER_CONSTRUCTION(new int[][] { { 33035 } }),
		MASTER_COOKING(new int[][] { { 33036 } }),
		MASTER_CRAFTING(new int[][] { { 33037 } }),
		MASTER_DEFENCE(new int[][] { { 33038 } }),
		MASTER_FARMING(new int[][] { { 33039 } }),
		MASTER_FIREMAKING(new int[][] { { 33040 } }),
		MASTER_FISHING(new int[][] { { 33041 } }),
		MASTER_FLETCHING(new int[][] { { 33042 } }),
		MASTER_HERBLORE(new int[][] { { 33043 } }),
		MASTER_HITPOINTS(new int[][] { { 33044 } }),
		MASTER_HUNTER(new int[][] { { 33045 } }),
		MASTER_MAGIC(new int[][] { { 33046 } }),
		MASTER_MINING(new int[][] { { 33047 } }),
		MASTER_PRAYER(new int[][] { { 33048 } }),
		MASTER_RANGE(new int[][] { { 33049 } }),
		MASTER_RUNECRAFTING(new int[][] { { 33050 } }),
		MASTER_SLAYER(new int[][] { { 33051 } }),
		MASTER_SMITHING(new int[][] { { 33052 } }),
		MASTER_STRENGTH(new int[][] { { 33053 } }),
		MASTER_THIEVING(new int[][] { { 33054 } }),
		MASTER_WOODCUTTING(new int[][] { { 33055 } }),
		MAX_CAPE(new int[][] { { 13280 } }),
		ARDOUGNE_MAX_CAPE(new int[][] { { 20760 } }),
		FIRE_MAX_CAPE(new int[][] { { 13329 } }),
		AVAS_MAX_CAPE(new int[][] { { 13337 } }),
		ASSEMBLER_MAX_CAPE(new int[][] { { 13337 } }),
		SARADOMIN_MAX_CAPE(new int[][] { { 21898 } }),
	...
	public static final int[] MAX_CAPE_IDS = { 13280, 13329, 21898, 13337, 13331, 13333, 13335, 20760, 21285 };
		
	public static final SkillcapePerks[] MAX_CAPES = { MAX_CAPE, ARDOUGNE_MAX_CAPE, FIRE_MAX_CAPE, AVAS_MAX_CAPE, SARADOMIN_MAX_CAPE, ZAMORAK_MAX_CAPE, GUTHIX_MAX_CAPE, INFERNAL_MAX_CAPE };
	
	public static final SkillcapePerks[] MASTER_CAPES = { MASTER_ATTACK, MASTER_AGILITY, MASTER_CONSTRUCTION, MASTER_COOKING, MASTER_CRAFTING, MASTER_DEFENCE,
			MASTER_FARMING, MASTER_FARMING, MASTER_FIREMAKING, MASTER_FISHING, MASTER_FLETCHING, MASTER_HERBLORE, MASTER_HITPOINTS, MASTER_HUNTER, MASTER_MAGIC,
			MASTER_MINING, MASTER_PRAYER, MASTER_RANGE, MASTER_RUNECRAFTING, MASTER_SLAYER, MASTER_SMITHING, MASTER_STRENGTH, MASTER_THIEVING, MASTER_WOODCUTTING };
	...
	public static boolean isWearingMasterCape(Player player) {
		if (MASTER_CAPES.length >= 0) {
			return true;
		}
		return false;
	}
	...
	public static void mixCape(Player player, String cape) {
		...
		case "INFERNAL":
			if (!player.getItems().playerHasItem(21295)) {
				player.sendMessage("You must have an infernal cape in order to do this.");
				return;
			}
		...
		case "AVAS":
			if (!player.getItems().playerHasItem(10499)) {
				player.sendMessage("You must have an accumulator in order to do this.");
				return;
			}
		...
		case "ASSEMBLER":
			if (!player.getItems().playerHasItem(22109)) {
				player.sendMessage("You must have an assembler in order to do this.");
				return;
			}
			player.getItems().deleteItem(22109, 1);
			player.getItems().addItem(21898, 1);
			player.getItems().addItem(21900, 1);
			player.getDH().sendItemStatement("You've combined the accumulator and max cape.", 21898);
			break;
