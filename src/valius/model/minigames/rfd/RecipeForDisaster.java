...
public final class RecipeForDisaster extends SingleInstancedArea {
	...
	public void init() {
		...
		//World.getWorld().getNpcHandler().spawnNpc(player, 6368, 1899, 5356, height, 0, 180, 25, 170, 240, false, false);
	...
	public void wave() {
		World.getWorld().getEventHandler().submit(new Event<Player>("rfd", player, 5) {
			...
			public void execute() {
				...
				World.getWorld().getNpcHandler().spawnNpc(player, WAVE_DATA[player.rfdWave][0], SPAWN_X, SPAWN_Y, height, 0, WAVE_DATA[player.rfdWave][1], WAVE_DATA[player.rfdWave][2],
	...
	public final void end(DisposeTypes dispose) {
		...
		player.getPA().movePlayer(3093, 3500, 0);
