...
import valius.model.minigames.bounty_hunter.events.TargetDelay;
import valius.model.minigames.bounty_hunter.events.TargetSelector;
 ...
 */
public class BountyHunter implements CycleEvent {
	/**
	...
	public void dropPlayerEmblem(Player killer) {
		...
		if (Misc.random(9) != 0 && killer.getBH().hasTarget() && hasTarget() && killer.playerName.equalsIgnoreCase(target.getName())) {
			if (emblem.isPresent()) {
				World.getWorld().getItemHandler().createGroundItem(killer, emblem.get().getPreviousOrFirst().getItemId(), player.getX(), player.getY(), player.getHeight(), 1, killer.getIndex());
			} else {
				if (Misc.random(3) != 0) {
					World.getWorld().getItemHandler().createGroundItem(killer, BountyHunterEmblem.TIER_1.getItemId(), player.getX(), player.getY(), player.getHeight(), Config.DOUBLE_PKP ? 2 : 1,
							killer.getIndex());
				}
			}
		}
	...
	public void teleportToTarget() {
		...
		if (player.getSkills().getLevel(Skill.MAGIC) < 85) {
		...
		for (int teleX = targetX - 1; teleX < targetX + 2; teleX++) {
			for (int teleY = targetY - 1; teleY < targetY + 2; teleY++) {
				if (!Region.canMove(teleX, teleY, teleX + 1, teleY + 1, target.getHeight(), 1, 1)) {
					player.sendMessage("Your target is in a blocked in area, you cannot teleport to them right now.");
					return;
				}
			}
		}
		player.getPA().startTeleport(targetX, targetY - 1, target.getHeight(), "modern", false);
