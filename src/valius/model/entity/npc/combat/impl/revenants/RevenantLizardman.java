...
@ScriptSettings(npcIds = { 3372 })

public class RevenantLizardman extends CombatScript {
	...
	public int attack(NPC npc, Entity target) {
		...
		if (target.asPlayer().protectingMelee()) {
			switch (randomAttack) {
				case 1:
					handleHit(npc, target, CombatType.MAGE, MAGE_PROJECTILE, null, new Hit(Hitmark.HIT, getRandomMaxHit(npc, target, CombatType.MAGE, 31), 3));
					break;
				case 2:
					handleHit(npc, target, CombatType.RANGE, RANGE_PROJECTILE, null, new Hit(Hitmark.HIT, getRandomMaxHit(npc, target, CombatType.RANGE, 31), 3));
					break;
			}
		} 
		 ...
		 */
			...
			switch (randomAttack) {
				case 1:
					handleHit(npc, target, CombatType.MELEE, new Hit(Hitmark.HIT, getRandomMaxHit(npc, target, CombatType.MELEE, 31), 1));
					break;
				case 2:
					handleHit(npc, target, CombatType.RANGE, RANGE_PROJECTILE, null, new Hit(Hitmark.HIT, getRandomMaxHit(npc, target, CombatType.RANGE, 31), 3));
					break;
