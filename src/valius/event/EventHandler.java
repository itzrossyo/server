...
public class EventHandler {
	...
	public <T> boolean isRunning(T attachment, Class<? extends Event<?>> eventClass) {
		Predicate<Event<?>> running = event -> event.isAlive() && event.getAttachment().equals(attachment) && event.getClass() == eventClass;
		return active.stream().anyMatch(running) || pendingAddition.stream().anyMatch(running);
	}
}
