...
import valius.clip.Tile;
import valius.clip.TileControl;
...
public class NPCDumbPathFinder {
	...
	public static Location walkTowards(NPC npc, int waypointx, int waypointy, boolean ignoreClipping) {
		...
		if (canMoveTo(npc, toDir) || ignoreClipping) {
			...
			if (toDir == 0) {
				if (canMoveTo(npc, 3) || ignoreClipping) {
					direction = 3;
				} else if (canMoveTo(npc, 1) || ignoreClipping) {
					direction = 1;
				}
			} else if (toDir == 2) {
				if (canMoveTo(npc, 1)) {
					direction = 1;
				} else if (canMoveTo(npc, 4) || ignoreClipping) {
					direction = 4;
				}
			} else if (toDir == 5) {
				if (canMoveTo(npc, 3) || ignoreClipping) {
					direction = 3;
				} else if (canMoveTo(npc, 6) || ignoreClipping) {
					direction = 6;
				}
			} else if (toDir == 7) {
				if (canMoveTo(npc, 4) || ignoreClipping) {
					direction = 4;
				} else if (canMoveTo(npc, 6) || ignoreClipping) {
					direction = 6;
				}
	...
	public static boolean canMoveTo(final NPC mob, final int direction) {
		...
		final int z = mob.getHeight() > 3 ? mob.getHeight() % 4 : mob.getHeight();
