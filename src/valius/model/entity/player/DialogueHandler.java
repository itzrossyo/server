...
import valius.content.IronManInterface.Action;
...
import valius.model.minigames.bounty_hunter.BountyHunterEmblem;
...
public class DialogueHandler {
	...
	public void sendDialogues(int dialogue, int npcId) {
		...
		case 354:
			this.sendNpcChat4("Hello " + c.playerName + ". I can give you back your Valius Armor", "if you lose it. Once you reach @blu@1000</col> total level", "i will give you a set of @blu@Tier 2</col> Valius armor", " which just happens to be the armor i'm wearing.", 3585, "Clarkeh [Valius Armor Upgrading]");
			c.nextChat = 355;
			break;
		case 355:
			sendOption3("I'd like a set of Tier 1 Valius armor", "I'd like a set of Tier 2 Valius armor", "Nevermind.");
			c.dialogueAction = 356;
				break;
				
		case 357:
			this.sendNpcChat4("Hello " + c.playerName + ". Check out @blu@::Dperks</col> before donating!", "You will receive a Mbox for donating $5+,", "a Valius Mbox for donating $25+", "and a Raids Mbox for donating $50+ at once!", 7204, "Donator Shopkeeper");
			c.nextChat = 358;
			break;
		case 358:
			this.sendNpcChat4("ALL items purchased with Tokens go towards your", "total amount donated no matter what you buy.", "Scrolls offer you donator status at a cheaper rate!", "Once you're ready, trade me to check out my shops!", 7204, "Donator Shopkeeper");
			c.nextChat = -1;
			break;
			
		case 359:
			this.sendNpcChat4("Items in my shop are only Available for a @red@LIMITED</col> time!", "Once an item is taken down from my shop, it will never", "be obtainable in-game again, making items purchased here", "Much more rare than your average items!", 3819, "Limited Time Donation Shopkeeper");
			c.nextChat = -1;
			break;
			
		case 10000:
			this.sendOption2("Would you like to leave the raid?", "Yes, I'd like to leave.", "No I want to stay.");
			c.dialogueAction = 10000;
			break;
			
		case 10001:
			this.sendOption2("Donator Thermonuclear Smoke devil dungeon", "Members Boss: Nightmare");
		...
		case 701:
			...
			if (c.getItems().playerHasItem(13273)) {
				...
				if (Misc.random(100) == 15) {
					...
					GlobalMessages.send(c.playerName + " received a pet from Abyssal Sire.", GlobalMessages.MessageType.LOOT);
				}
				switch (reward) {
				case 13274:
				...
				GlobalMessages.send(c.playerName+" received a drop: "+ItemAssistant.getItemName(reward), GlobalMessages.MessageType.LOOT);
				DiscordBot.sendMessage("valius-bot", Misc.capitalize(c.playerName) + " received a drop: "
		...
		case 11929:
			sendNpcChat3("Hello " + c.playerName + " I am the OSRS GP Donation Manager.",
					"If you are interested in donating, contact Divine.",
					"Donate to anyone else you will be perm banned!", 3189, "GP Donation Manager");
			c.nextChat = 11930;
			break;

		case 11930:
			sendNpcChat2("The current exchange rates are:", "@red@OSRS Gold: @blu@1.2m = $1.00", 3189,
					"GP Donation Manager");
			c.nextChat = -1;
			break;
		...
		case 1:
			sendNpcChat2("Come back when you have finished all your achievements.", "", 5527, "Achievement Handler");
			c.nextChat = -1;
			break;
		case 2:
			sendNpcChat2("This cape costs 99.000gp.", "", 5527, "Achievement Handler");
			c.nextChat = -1;
			break;
		case 4:
			...
			break;
		case 7:
			sendNpcChat2("There you go, good job.", "", 5027, "Achievement Handler");
			c.nextChat = -1;
			break;
		...
		case 3300:
			if (c.getSlayer().getTask().isPresent()) {
				if (c.getSlayer().getTask().get().getLevel() > c.getSkills().getLevel(Skill.SLAYER)) {
		...
		case 5998:
			sendNpcChat3("Only <img=4>@red@Donators @bla@can access the Donator Zone.",
					"Interested in becoming a Valius <img=4>@red@Donator@bla@?",
		...
		case 55872:
			...
			break;
		case 55873:
			sendNpcChat2("Hello, my name is Banky.", "I was hired by Valius to be a travelling banker!", c.talkingNpc,
					"Banky The Banker");
			c.nextChat = 0;
			break;
		case 55875:
		...
		case 2400:
			sendOption5("Imbue Berserker Ring - @red@1000 Bloodmoney", "Imbue Archer Ring - @red@1000 Bloodmoney",
					"Imbue Seers Ring - @red@1000 Bloodmoney", "Imbue Warrior Ring - @red@1000 Bloodmoney", "More");
			c.dialogueAction = 114;
			c.teleAction = -1;
			c.nextChat = 0;
			break;
		case 2401:
			c.getDH().sendNpcChat2("Hello. I can take one of your regular rings", "and imbue it for 1000 Bloodmoney.",
					c.talkingNpc, "Amik Varze");
			c.nextChat = 2400;
			break;

		case 2402:
			sendOption5("Imbue Treasonous Ring @red@1000 Bloodmoney", "Imbue Tyrannical Ring @red@1000 Bloodmoney", "", "Previous",
					"Close");
		...
		case 512:
			sendNpcChat1("If you help me find those pumpkins, I'll let you live.", c.talkingNpc,
					"Grim Reaper");
			c.nextChat = 514;
			break;

		case 513:
			sendOption3("No...", "Do I really have a choice?", "Yes");
		...
		case 532:
			if (c.getHolidayStages().getStage("Halloween") == 4) {
				if (c.getItems().playerHasItem(1960, 50)) {
					...
					World.getWorld().getHolidayController().giveReward(c, HolidayController.HALLOWEEN);
				} else {
		...
		case 554:
			sendNpcChat3("I'll happily take those off of your hands for a handsome", "fee.", "You currently have " + c.getBH().getBounties() + "Bounty hunter points.", c.talkingNpc,
					"Emblem Trader");
			c.nextChat = 555;
			break;
		...
		case 587:
			sendNpcChat("Alright then, come back if you do.", "Merry Christmas!");
			c.nextChat = -1;
			break;
		...
		case 589:
			sendNpcChat("Each and every year presents are made by the elves",
					...
					"Presents are made right here on this mountain, hidden from",
					"all of the Valius Players!");
			c.nextChat = 590;
			break;

		case 590:
			sendNpcChat("Usually we disguise our workshop with that boring",
					"monestary but we have a problem...",
					"That jealous old Anti-Santa has invaded my mountain!",
					"we can no longer make toys in peace");
			c.nextChat = 591;
			break;

		case 591:
			sendNpcChat("The toys that the elves have already made have",
					"been stolen by Anti-Santa's Imps..");
			c.nextChat = 592;
			break;

		case 592:
			sendNpcChat("I need you to slay these vile imps and return",
					"the presents to me!");
			c.nextChat = 593;
			break;

		case 593:
			sendNpcChat("Of course, I'll reward you for returning them to me",
					"Just put the presents on my table over there.");
			c.nextChat = 594;
			break;

		case 594:
			sendNpcChat("Also..",
					"Anti-Santa himself is hiding out around Valius,",
					"Be on the look-out for him he may have taken some",
					"of my more rare treasures...");
			c.nextChat = 596;
			break;
		...
		case 598:
			sendNpcChat("Great! Good luck and stay safe!");
			c.nextChat = -1;
			break;

		case 599:
			sendNpcChat("I still don't have all of the presents back yet!",
					"",
					"Keep Searching!",
					"");
			c.nextChat = -1;
		...
		case 600:
			sendOption2("I don't know what to do.",	"Nevermind");
			c.dialogueAction = 118;
			break;

		case 612:
			sendOption2("Use christmas cracker, I may get a partyhat.", "No, I want to keep my cracker.");
		...
		case 623:
			...
			if (DegradableItem.forId(degradeList[0].getItemId()).isPresent())
				item1 = ItemAssistant.getItemName(degradeList[0].getItemId()) + " @red@"
						+ (DegradableItem.forId(degradeList[0].getItemId()).get().getCost() * 2) + "Coins";
			...
			if (degradeList.length > 1 && DegradableItem.forId(degradeList[1].getItemId()).isPresent()) {
				item2 = ItemAssistant.getItemName(degradeList[1].getItemId()) + " @red@"
						+ (DegradableItem.forId(degradeList[1].getItemId()).get().getCost() * 2) + " Coins";
			}
			if (degradeList.length > 2 && DegradableItem.forId(degradeList[2].getItemId()).isPresent()) {
				item3 = ItemAssistant.getItemName(degradeList[2].getItemId()) + " @red@"
						+ (DegradableItem.forId(degradeList[2].getItemId()).get().getCost() * 2) + " Coins";
			}
			if (degradeList.length > 3 && DegradableItem.forId(degradeList[3].getItemId()).isPresent()) {
				item4 = ItemAssistant.getItemName(degradeList[3].getItemId()) + " @red@"
						+ (DegradableItem.forId(degradeList[3].getItemId()).get().getCost() * 2) + " Coins";
			}
		...
		case 644:
			SerializablePair<String, Long> pair = World.getWorld().getServerData().getZulrahTime();
		...
		case 645:
			c.getPA().movePlayer(3094, 3500, 0);
			sendNpcChat("You've made it through!", "You have the option to play as an <img=12></img> Iron Man,",
					"<img=13></img> Ultimate Iron Man, or Hardcore Iron Man.", "Choose from the following interface.");
			c.nextChat = 646;
			break;
		...
		case 647:
			...
			} else if (c.getMode().getType().equals(ModeType.HC_IRON_MAN)) {
				sendNpcChat("You have chosen the Hardcore Ironman.");
			} else {
				sendNpcChat("You have chosen the mode " + c.getMode().getType().toString() + ".",
						"Take this set of armour to help you on your way.", "Speak with me for more information if needed!");
			}
			c.nextChat = 648;
			break;
		...
		case 649:
			sendNpcChat("Now that you have chosen your Mode & Exp Rate", "You can now start your adventure! Good luck.");
		...
		case 661:
			...
			EquipmentSet set =
					  c.getMode().isIronman() ? EquipmentSet.IRON_MAN_ARMOUR
					: c.getMode().isUltimateIronman() ? EquipmentSet.ULTIMATE_IRON_MAN_ARMOUR
					: c.getMode().isUltimateIronman() ? EquipmentSet.GROUP_IRON_MAN_ARMOUR
					: c.getMode().isHcIronman() ? EquipmentSet.HC_IRON_MAN_ARMOUR :
						null;
			...
			for (int[] equipment : set.getEquipment()) {
				for (int item : equipment) {
					if (World.getWorld().getItemHandler().itemExists(c.playerName, item)) {
		...
		case 672:
			sendOption4("Normal: 30x exp rates | 125x combat exp rates", "Extreme: 15x exp rates, 5% droprate increase",
					"Classic: 5x exp rates, 10% droprate increase", "Elite: 2x exp rates, 15% droprate increase");
			c.dialogueAction = 716;
			break;

		case 673:
			sendNpcChat("Glad to see you have joined Valius, The #1 Custom Server", "With TONS of unique content to explore and obtain.",
					"From FULL Theatre of Blood to Unique custom content,", "There is something for everyone here.");
			c.nextChat = 674;
			break;

		case 674:
			c.getPA().resetCinematicCamera();
			c.getPA().movePlayerUnconditionally(3079, 3493, 0);
		...
		case 676:
			c.getPA().movePlayerUnconditionally(3076, 3513, 0);
			sendNpcChat("Slayer masters are located here.", "If you would like to do Wilderness Slayer",
					"head directly east and talk to Krystillia", "or speak with Nieve for Boss slayer tasks.");
			c.nextChat = 10100;
			break;
			
		case 10100:
			sendNpcChat("You will also find the @blu@ Slayer chest</col> here.", "This chest has 4 tiers of keys to obtain",
					"and rewards such as Slayers Armor, Rare boss drops and", "Skilling supplies while opening it. @blu@Higher tier = better loot!");
			c.nextChat = 10111;
			break;
			
		case 10111:
			sendNpcChat("Also, every slayer monster including ","Superior monsters drop pets! This goes for all bosses as well.","Some pets even offer Droprate increases!");
			c.nextChat = 677;
			break;

		case 677:
			c.getPA().movePlayerUnconditionally(3080, 3511, 0);
			sendNpcChat("This is the Wilderness hub.", "You can find useful npcs and tools",
					"here like Nurse Boubou", "and the emblem trader!");
			c.nextChat = 10101;
			break;
			
		case 10101:
			c.getPA().movePlayerUnconditionally(3090, 3499, 0);
			sendNpcChat("Players can spend @blu@Boss points</col> received from all bosses,", "@blu@PVM points</col> receive from all monsters,",
					"@blu@Skill points</col> received randomly while training skills", "And @blu@Bloodmoney</col> which is obtained from most Events.");
			c.nextChat = 10116;
			break;
			
		case 10116:
			sendNpcChat("Some items like the @blu@Bloodmoney Mystery box</col> have ", "great rewards such as the @blu@bloodknight armor</col> which",
					"Heals you depending on how many pieces worn and", "@blu@Blood perks</col> for the @blu@Twisted Crossbow</col>.");
			c.nextChat = 10102;
			break;
			
		case 10102:
			c.getPA().movePlayerUnconditionally(3097, 3499, 0);
			sendNpcChat("Once you hit 1,000 total level you can speak", "with @blu@Clarkeh</col> to upgrade your starter armor.",
					"The starter armor provides a little XP boost as well as", "BONUS Vote points to spend while wearing it and voting.");
			c.nextChat = 10103;
			break;
			
		case 10103:
			sendNpcChat("You will recieve @blu@Bonus XP</col> when voting each day as well as", "a 1:10 chance to receive double points + a Valius Mbox!",
					"There is also a Monthly top voter Event with some amazing awards.", "Check the ::Forums for more info on that!");
			c.nextChat = 10104;
			break;
			
		case 10104:
			c.getPA().movePlayerUnconditionally(3100, 3503, 0);
			sendNpcChat("The Infernal Chest here can be unlocked by using", "@blu@Infernal Keys</col>. These are obtained by collecting",
					"The 3 pieces randomly dropped by all monsters and bosses", "Then creating the key and using it on the chest.");
			c.nextChat = 10105;
			break;
			
		case 10105:
			sendNpcChat("Some rewards from the chest are:", "@blu@God Bows, Infernal Mystery boxes + Infernal cape recolors,",
					"@blu@ and Colossal Armor which provides Burn dmg", "to each hit, hitting higher with more pieces.");
			c.nextChat = 10109;
			break;
			
		case 10109:
			c.getPA().movePlayerUnconditionally(3089, 3488, 0);
			sendNpcChat("Valius has a fully @blu@customizable completionist cape</col>.", "You can check requirements by clicking the cape rack here",
					"This cape is the best cape in-game, offering The stats", "of the @blu@Infernal cape, Imbued god cape & avas assembler</col> in one!");
			c.nextChat = 10114;
			break;
			
		case 10114:
			sendNpcChat("We also have @blu@Master capes</col> for achieving 200M XP", "In any skill. These capes have nice particle effects",
					"At the bottom and offer some great bonuses", "such as @blu@10% to your Droprates</col>.");
			c.nextChat = 10115;
			break;
			
		case 10115:
			sendNpcChat("You can speak to the Wise old man just to ", "Your left to purchase those capes as well",
					"as regular skillcapes. Speak with Max", "once you receive 99 in all skills for the @blu@Maxcape</col>.");
			c.nextChat = 678;
			break;

		case 678:
			c.getPA().movePlayerUnconditionally(3075, 3476, 0);
			sendNpcChat("Want to make a little starter money?", "Steal from the market stalls and",
					"sell the goods to Sigmund the Merchant.", "@blu@Raids</col> are a great source of income later!");
			c.nextChat = 10117;
			break;
		case 10117:
			sendNpcChat("The @blu@magnifying glass</col> in the bottom right of your screen", "holds your @blu@achievements tracker</col> and from here",
					 "you can unlock great rewards such as @blu@skilling outfits", "Like the rogue outfit which offers @blu@bonus XP</col> to thieving.");
			c.nextChat = 679;
			break;
		case 679:
			c.getPA().movePlayerUnconditionally(3109, 3488, 0);
			sendNpcChat("This area has tons of useful things like", "the RFD Miniquest and the Occult Altar for swapping spellbooks,  ",
					 "Attack dummies to test your Max Hit and", "Knights to exchange Zulrah items for Scales.");
			c.nextChat = 680;
			break;
		case 680:
			c.getPA().movePlayerUnconditionally(3088, 3466, 0);
			sendNpcChat("This is the home resource area. It's located","South of the Home Area.","It's got all sorts of basic nodes for","Fishing, Mining, and Woodcutting!");
			c.nextChat = 10118;
			break;
		case 10118:
			c.getPA().movePlayerUnconditionally(3294, 9501, 0);
			sendNpcChat("This is the Starter dungeon. You can get lots","of unique drops here and is a great place to",
					"start your adventure. Speak with the @blu@starter dungeon guide</col>", "to learn all about the dungeon and its rewards!");
			c.nextChat = 681;
			break;
		case 681:
			c.getPA().movePlayerUnconditionally(3060,9951,0);
			sendNpcChat("This is our Minigame Lobby!","We have Raids 1 as well as a","custom Minigame, Trials of Xeric!","and Theatre of Blood nearly identical to OSRS.");
			c.nextChat = 10106;
			break;
			
		case 10106:
			c.getPA().movePlayerUnconditionally(1352, 10258,0);
			sendNpcChat("Players can fight some epic bosses like","The Alchemical Hydra which is 100% replicated like OSRS","You can also check out the rest of the Karluum dungeon",
					"in the Monster teleports and fight Drakes, Hydras and Wyrms!");
			c.nextChat = 10107;
			break;
			
		case 10107://
			c.getPA().movePlayerUnconditionally(3030, 6123,1);
			sendNpcChat("For those who enjoy Dungeoneering we have The Gauntlet","Which is a OSRS minigame that generates a random dungeon","for players to explore and collect supplies to create",
					"Crystal armor and weapons and defeat the Boss at the end.");
			c.nextChat = 10108;
			break;
			
		case 10108://
			sendNpcChat("Rewards such as Crystal armor, Crystal Tools and", "the Blade of saeldor can be obtained.");
			c.nextChat = 10112;
			break;
			
		case 10112://
			c.getPA().movePlayerUnconditionally(1578, 3952, 0);
			sendNpcChat("We also have a ton of @blu@Custom bosses</col> like The ","Void champion and Nightmare as well as Hourly event bosses"," and Wilderness event bosses every half an hour.");
			c.nextChat = 10113;
			break;
			
		case 10113://
			sendNpcChat("Defeating some of these bosses can get you Infernal weapons,", "Chaotics, Supreme void armor + the Korasi & Grotesque armor", "as well as some perks to be used",
					    "on twisted bows, armor and more!");
			c.nextChat = 690;
			break;
			
		case 690:
			c.getPA().movePlayerUnconditionally(3094, 3496, 0);
			sendNpcChat("Finally, to teleport, simply use", "this portal at home and select your destination.");
			c.nextChat = 645;
			break;
		...
		case 163:
			...
			if (c.getSkills().getLevel(Skill.MAGIC) >= 60) {
				c.nextChat = 164;
			} else {
				c.nextChat = 0;
		...
		case 23682763:
			sendNpcChat("Now that was quite the show!", "I haven't been that entertained in a long time.");
			c.nextChat = 23682764;
			break;
		...
		case 23682765:
			c.getDH().sendOption2("Is your party ready to fight?", "Yes, let's begin.", "No don't start yet.");
			c.dialogueAction = 23682765;
			break;
			
		case 23682766:
			sendPlayerChat1("Yes let's begin.");
			c.nextChat = 0;
			break;
			
		case 23682767:
			sendPlayerChat1("No don't start yet.");
			c.nextChat = 0;
			break;
	...
	public void sendNpcChat(String... messages) {
		String name = Misc.capitalize(World.getWorld().getNpcHandler().getNpcName(c.talkingNpc));
