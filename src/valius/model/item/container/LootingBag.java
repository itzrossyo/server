...
public class LootingBag {
    ...
    public static boolean isLootingBag(Player player, int itemId) {
        return itemId == LOOTING_BAG_OPEN || itemId == LOOTING_BAG_CLOSED;
    }
    
    public boolean hasLootBag() {
    	return player.getItems().playerHasItem(LOOTING_BAG_CLOSED) || player.getItems().playerHasItem(LOOTING_BAG_OPEN);
    ...
    public boolean allowedInBag(int item) {
        switch (item) {
            case LOOTING_BAG_OPEN:
            case LOOTING_BAG_CLOSED:
            case 11942:
                player.sendMessage("You may be surprised to learn that bagception is not permitted.");
                return false;
            default:
                if (ItemDefinition.forId(item) == null || !ItemDefinition.forId(item).isTradable()) {
    ...
    public void sendItems() {
        ...
        for (int i = 0; i < 28; i++) {
            if (i < items.size()) {
                ...
                if (item != null) {
                    loot.add(new Item(item.getId(), item.getAmount()));
                }
            }
