...
public class Impling {
	...
	public static void catchImpling(Player player, int npcId, final int npcIndex) {
		...
		if (player.getSkills().getLevel(Skill.HUNTER) < data.requirement) {
			player.sendMessage("You need a hunter level of at least " + data.requirement + " to catch this impling.");
			return;
		}
		boolean bareHanded = player.getSkills().getLevel(Skill.HUNTER) > data.requirement + 10;
		...
		boolean fail = Misc.random(10) >= ((player.getSkills().getLevel(Skill.HUNTER) - 10) / 10) + 1;
		if (!fail) {
			...
			player.getPA().addSkillXP(data.experience, 21, true);
		} else {
	...
	public enum ImpRewards {
		...
		REVENANT(33773, new int[][] { 
			...
			{1392, 1, 5}, {560, 70, 100}, {565, 70, 100}, {563, 70, 100}, {9193, 50, 100}, 
			{9194, 50, 100}, {454, 150, 200}, {302, 30, 60}, {1514, 10, 30}, {1748, 10, 20}, 
			{452, 5, 10}, {2364, 5, 10}, {2362, 5, 10},{21820, 5, 30}, {1201, 1}, {1163, 1}, {1127, 1}, {1079 , 1}, 
			{1347, 1}, {4087, 1}, {4585,1}, {1215,1}, {1305, 1}, 
			{1392, 1, 5}, {560, 70, 100}, {565, 70, 100}, {563, 70, 100}, {9193, 50, 100}, 
			{9194, 50, 100}, {454, 150, 200}, {302, 30, 60}, {1514, 10, 30}, {1748, 10, 20}, 
			{452, 5, 10}, {2364, 5, 10}, {2362, 5, 10}
		...
		public static void getReward(Player c, int itemId) {
			...
			if (itemId == 19732) {
				...
				if (randomClue >= 0 && randomClue <= 500) {
					c.getItems().addItem(Misc.randomElementOf(ClueScrollRiddle.EASY_CLUES), 1);
					c.sendMessage("x1 Easy clue");
				} else if (randomClue >= 501 && randomClue <= 850) {
					c.getItems().addItem(Misc.randomElementOf(ClueScrollRiddle.MEDIUM_CLUES), 1);
					c.sendMessage("x1 Medium clue");
				} else if (randomClue >= 851 && randomClue <= 990) {
					c.getItems().addItem(Misc.randomElementOf(ClueScrollRiddle.HARD_CLUES), 1);
					c.sendMessage("x1 Hard clue");
				} else if (randomClue >= 991 && randomClue <= 1000) {
					c.getItems().addItem(Misc.randomElementOf(ClueScrollRiddle.ELITE_CLUES), 1);
					c.sendMessage("x1 Elite clue");
				}
				return;
			}
