package valius.model.entity.player.skills.prayer;
...
public class Prayer {
	...
	public void bury(Bone bone) {
		...
		if(player.getItems().isWearingItem(22111)){
			...
			player.getSkills().increasLevelOrMax(prayerRestore, Skill.PRAYER);
			player.getPA().refreshSkill(5);
		}
		player.sendMessage("You bury the " + (definition == null ? "bone" : definition.getName()) + ".");
		player.getPA().addSkillXP(bone.getExperience() * (Boundary.isIn(player, Boundary.LAVA_DRAGON_ISLE) && bone.getItemId() == 11943 ? 16 : 1), Skill.PRAYER.getId(), true);
	...
	public void alter(final int amount, final WorldObject worldObject) {
		...
		ItemDefinition definition = ItemDefinition.forId(bone.getItemId());
		player.getPA().stillGfx(624, player.getX(), player.getY(), player.getHeight(), 1);
		player.getPA().addSkillXP(bone.getExperience() * 3, Skill.PRAYER.getId(), true);
		player.getItems().deleteItem2(bone.getItemId(), 1);
		player.startAnimation(713);
		lastAction.reset();
		lastAction.start();
		player.getSkilling().setSkill(Skill.PRAYER);
		World.getWorld().getEventHandler().submit(new Event<Player>("skilling", player, 3) {
