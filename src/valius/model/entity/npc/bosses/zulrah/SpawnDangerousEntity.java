...
public class SpawnDangerousEntity implements CycleEvent {
	...
	public void execute(CycleEventContainer container) {
		...
		if (location == null) {
			container.stop();
			zulrah.getNpc().faceEntity(player.getIndex());
			return;
		}
		if (ticks == 4 || ticks == 8 || ticks == 12 || ticks == 16) {
			for (Point point : location.getPoints()) {
				if (entity.equals(DangerousEntity.TOXIC_SMOKE)) {
					World.getWorld().getGlobalObjects().add(new GlobalObject(11700, point.x, point.y, zulrah.getInstancedZulrah().getHeight(), 0, 10, duration, -1));
				} else if (entity.equals(DangerousEntity.MINION_NPC)) {
					World.getWorld().getNpcHandler().spawnNpc(player, Zulrah.SNAKELING, point.x, point.y, zulrah.getInstancedZulrah().getHeight(), 0, 1, 11, 100, 50, true, false);
				}
			}
			locations.remove();
		}
