...
import valius.content.barrows.BrotherEvent;
...
public abstract class Brother {
	...
	public void spawnBrother() {
		...
		active = true;
		if (finalBrother) {
			npc = World.getWorld().getNpcHandler().spawnNpc(player, getId(), 3551, 9694, 0, 0, getHP(), getMaxHit(), getAttack(), getDefense(), true, true);
		} else {
			npc = World.getWorld().getNpcHandler().spawnNpc(player, getId(), getSpawn().getX(), getSpawn().getY(), player.getHeight(), 0, getHP(), getMaxHit(), getAttack(), getDefense(), true,
					true);
			World.getWorld().getEventHandler().submit(new BrotherEvent(player, 1));
		}
		if (npc != null) {
	...
	public abstract Location getStairsLocation();
