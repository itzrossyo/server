...
public abstract class MultiplayerSession implements MultiplayerSessionItemDistribution, MultiplayerSessionLog {
	 ...
	 */
	protected Map<Player, List<Item>> items = new HashMap<>();

	/**
	 * A map containing a list of game items the player has before the trade starts
	 */
	protected Map<Player, List<Item>> presetItems = new HashMap<>();
	...
	public MultiplayerSession(List<Player> players, MultiplayerSessionType type) {
		...
		this.type = type;
		players.stream().forEach(player -> items.put(player, new ArrayList<Item>()));
	}

	public abstract void accept(Player player, Player recipient, int stage);

	public abstract boolean itemAddable(Player player, Item item);

	public abstract boolean itemRemovable(Player player, Item item);
	...
	public void addItem(Player player, Item item) {
		...
		if (World.getWorld().getMultiplayerSessionListener().containsSessionInconsistancies(player)) {
			return;
		}
		...
		if (item.isStackable() && listContainsItem) {
			for (Item i : items.get(player)) {
				if (i.getId() == id) {
					...
					if (total > Integer.MAX_VALUE) {
						...
						items.get(player).add(new Item(id, Integer.MAX_VALUE));
					} else {
						items.get(player).remove(i);
						items.get(player).add(new Item(id, i.getAmount() + amount));
			...
			items.get(player).add(new Item(id, amount));
			player.getItems().deleteItem2(id, amount);
		} else {
			while (amount-- > 0) {
				items.get(player).add(new Item(id, 1));
	 ...
	 */
	public void removeItem(Player player, int slot, Item item) {
		int id = item.getId();
		int amount = item.getAmount();
		if (Objects.isNull(player) || Objects.isNull(item)) {
			return;
		...
		List<Item> items = this.items.get(player);
		...
		if (item.isStackable()) {
			for (int i = 0; i < items.size(); i++) {
				if (items.get(i).getId() == id) {
					...
					if (!presetListContains(player, id, amount) || inventoryContainsIllegalItem(player, new Item(id, amount))) {
						items.remove(i);
					...
					if (items.get(i).getAmount() - amount > 0) {
						if (player.getItems().addItem(id, amount)) {
							...
							items.set(i, new Item(id, previousAmount - amount));
						}
					} else {
						if (player.getItems().addItem(id, amount)) {
							items.remove(i);
	...
	public void accept(Player player, int stageId) {
		...
		for (Player p : players) {
			Iterator<Item> itemList = items.get(p).iterator();
	...
	public void finish(MultiplayerSessionFinalizeType type) {
		if (stage.getStage() == MultiplayerSessionStage.FINALIZE) {
			World.getWorld().getMultiplayerSessionListener().remove(this);
			return;
		}
		...
		for (Player player : players) {
			...
			for (int i = 0; i < player.playerItems.length; i++) {
				...
				if (itemId > 0 && amount > 0) {
					realItems.add(new Item(itemId, amount));
				}
			}
	...
	protected boolean presetListContains(Player player, int itemId, int amount) {
		if (!new Item(itemId, amount).isStackable()) {
			Optional<Item> op = presetItems.get(player).stream().filter(i -> i.getId() == itemId).findFirst();
			return op.isPresent();
		}
		Optional<Item> op = presetItems.get(player).stream().filter(i -> i.getId() == itemId && i.getAmount() >= amount).findFirst();
		return op.isPresent();
	}
	 ...
	 */
	protected boolean inventoryContainsIllegalItem(Player player, Item i) {
		long amount;
		if (!i.isStackable()) {
			amount = presetItems.get(player).stream().filter(item -> item.getId() == i.getId()).count();
		} else {
			Optional<Item> op = presetItems.get(player).stream().filter(item -> item.getId() == i.getId()).findAny();
			if (!op.isPresent()) {
				return true;
			}
	 ...
	 */
	protected Item getOverlappedItem(Player player) {
		Player recipient = getOther(player);
		for (Item playerItem : items.get(player)) {
			...
			if (amount > Integer.MAX_VALUE) {
				return new Item(playerItem.getId(), recipient.getItems().getItemAmount(playerItem.getId()));
			}
		}
		return null;
	}
	 ...
	 */
	protected List<Item> getItems(Player player) {
