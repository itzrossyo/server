...
public class GnomeAgility {
	...
	public boolean gnomeCourse(Player c, int objectId) {
		...
		case LOG_OBJECT:
			...
			} else if (c.getX() == 2474 && c.getY() > 3429 && c.getY() < 3436) {
		...
		case ROPE_OBJECT:
			...
			} else if (c.getY() == 3420 && c.getX() > 2477 && c.getX() < 2483) {
		...
		case PIPES1_OBJECT:
			if (c.getAgilityHandler().hotSpot(c, 2484, 3430)) {
				...
				c.getAgilityHandler().lapFinished(c, 5, 87, 4000);
			} else if (c.getY() > 3430 && c.getY() < 3436 && System.currentTimeMillis() - clickTimer > 1800) {
		...
		case PIPES2_OBJECT:
			if (c.getAgilityHandler().hotSpot(c, 2487, 3430)) {
				...
				c.getDiaryManager().getWesternDiary().progress(WesternDiaryEntry.GNOME_AGILITY);
				c.getAgilityHandler().lapFinished(c, 5, 87, 4000);
			} else if (c.getY() > 3430 && c.getY() < 3436) {
