...
import valius.event.impl.PoisonEvent;
import valius.event.impl.PoisonResistanceEvent;
import valius.event.impl.VenomEvent;
import valius.event.impl.VenomResistanceEvent;
...
public final class Health {
	...
	public void proposeStatus(HealthStatus status, int damage, Optional<Entity> inflictor) {
		...
		if (World.getWorld().getEventHandler().isRunning(entity, "health_status")) {
			World.getWorld().getEventHandler().stop(entity, "health_status");
		}

		if (status.isVenomed()) {
			World.getWorld().getEventHandler().submit(new VenomEvent(entity, damage, inflictor));
		}

		if (status.isPoisoned()) {
			World.getWorld().getEventHandler().submit(new PoisonEvent(entity, damage, inflictor));
		}
		...
		if (entity instanceof Player) {
			...
			player.sendMessage("You have been effected by " + status.toString().toLowerCase() + ".");
	...
	public void resolveStatus(HealthStatus status, int resistanceDuration) {
		...
		if (status == HealthStatus.POISON) {
			World.getWorld().getEventHandler().stop(entity, "poison_resistance_event");
			World.getWorld().getEventHandler().submit(new PoisonResistanceEvent(entity, resistanceDuration));
		}

		if (status == HealthStatus.VENOM) {
			World.getWorld().getEventHandler().stop(entity, "venom_resistance_event");
			World.getWorld().getEventHandler().submit(new VenomResistanceEvent(entity, resistanceDuration));
		}
	...
	private void requestUpdate() {
		if (entity instanceof Player) {
			...
			player.getSkills().setLevel(amount, Skill.HITPOINTS);
			player.getSkills().sendRefresh(Skill.HITPOINTS); 
		}
	}
	...
	public double getPercentage() {
		return (double) amount / (double) maximum;
	}
...
//		if (entity instanceof Player) {
//			Player player = (Player) entity;
//			World.getWorld().getNpcHandler().ringOfLife(player);
//		}
		if (amount <= 0) {
			return;
		}
