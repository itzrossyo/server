...
import valius.model.entity.player.combat.effects.AmuletOfTheDamnedDharokEffect;
import valius.model.entity.player.combat.effects.AmuletOfTheDamnedKarilEffect;
...
import valius.model.entity.player.combat.specials.Shove;
...
public class AttackPlayer {
	...
	public static void applyPlayerMeleeDamage(Player c, int i, int damageMask, int damage, Hitmark hitmark) {
		...
		case 3:
			if (damage > 0) {
				o.getSkills().decreaseLevelOrMin(damage, Skill.ATTACK);
				o.sendMessage("You feel weak.");
				o.getSkills().sendRefresh();
			}
			break;
		case 4:
			if (damage > 0) {
		...
		case 5:
			if (c.playerIndex > 0) {
				if (damage > 0) {
					c.getSkills().increasLevelOrMax(damage, Skill.PRAYER);
					o.getSkills().decreaseLevelOrMin(damage, 0, Skill.PRAYER);
	...
	public static void addCombatXP(Player c, CombatType type, int damage) {
		...
		default:
		case MELEE:
			experience += (int) Math.ceil((damage * 4)) / 3;
			skills.add(Skill.HITPOINTS.getId());
			c.getPA().addSkillXP((int) Math.ceil((damage * 4)) / 3, 3, false);

			switch (c.fightMode) {
			default:
			case 0: // Accurate
				skills.add(Skill.ATTACK.getId());
				experience += (int) Math.ceil(damage * 4);
				c.getPA().addSkillXP((int) Math.ceil(damage * 4), 0, false);
				break;
			case 1: // Block
				skills.add(Skill.DEFENCE.getId());
				experience += (int) Math.ceil(damage * 4);
				c.getPA().addSkillXP((int) Math.ceil(damage * 4), 1, false);
				break;
			case 2: // Aggressive
				skills.add(Skill.STRENGTH.getId());
				experience += (int) Math.ceil(damage * 4);
				c.getPA().addSkillXP((int) Math.ceil(damage * 4), 2, false);
				break;
			case 3: // Controlled
				int split = (int) Math.ceil(damage * 4) / 3;
				experience += (int) Math.ceil(damage * 4);
				for (int i = 0; i < 3; i++) {
		...
		case RANGE:
			skills.add(Skill.HITPOINTS.getId());
			experience += (damage * 4) / 3;
			c.getPA().addSkillXP((damage * 4) / 3, 3, false);
			switch (c.fightMode) {
			case 0: // Accurate
			case 2: // Rapid
				skills.add(Skill.RANGED.getId());
				experience += (damage * 4);
				c.getPA().addSkillXP(damage * 4, 4, false);
				break;
			case 1: // Block
			case 3:
				skills.add(Skill.RANGED.getId());
				skills.add(Skill.DEFENCE.getId());
				experience += (damage * 4) + (damage * 4 / 2);
				c.getPA().addSkillXP(damage * 4, 1, false);
				c.getPA().addSkillXP(damage * 4 / 2, 4, false);
				break;
			}
			break;
		case MAGE:
			...
			experience += ((damage * 4) / 3) + (damage * 4);
			c.getPA().addSkillXP((damage * 4) / 3, 3, false);
			c.getPA().addSkillXP(damage * 4, 6, false);
			break;
		}
		if (experience > 0) {
	...
	public static void playerDelayedHit(final Player c, final int targetId, final Damage d) {
		if (PlayerHandler.players[targetId] != null) {
			Player target = PlayerHandler.players[targetId];
			if (target == null || target.isDead || c.isDead || PlayerHandler.players[targetId].getHealth().getAmount() <= 0 || c.getHealth().getAmount() <= 0) {
				c.playerIndex = 0;
				return;
			}
			...
			if (d.getSpecial() != null) {
				d.getSpecial().hit(c, target, d);
			}

			if (d.getCombatType() == CombatType.MELEE) {
				if (World.getWorld().getEventHandler().isRunning(target, "staff_of_the_dead")) {
					Special special = Specials.STAFF_OF_THE_DEAD.getSpecial();
					special.hit(target, c, d);
				}
			}
			...
			if (target.playerIndex <= 0 && target.npcIndex <= 0) {
				if (target.autoRet == 1) {
					target.playerIndex = c.getIndex();
					target.followId = c.getIndex();
				}
			}
			if (c.hasOverloadBoost) {
				c.getPotions().resetOverload();
			}
			if (target.attackTimer <= 3 || target.attackTimer == 0 && target.playerIndex == 0 && !c.usingMagic) { // block
																									// animation
				target.startAnimation(target.getCombat().getBlockEmote());
			}
			if (c.projectileStage == 0 && !c.usingMagic) { // melee hit damage
				c.getCombat().applyPlayerHit(c, targetId, d);
			}
			DamageEffect effect = new AmuletOfTheDamnedDharokEffect();
			if (effect.isExecutable(target)) {
				effect.execute(c, target, d);
			}
			DamageEffect venom = new SerpentineHelmEffect();
			if (venom.isExecutable(target)) {
				venom.execute(c, target, new Damage(6));
			}
			...
			if (!c.usingMagic && c.projectileStage > 0) { // range hit damage
				...
				if (karilEffect.isExecutable(c)) {
					karilEffect.execute(c, target, d);
				}
				if (target.vengOn) {
					c.getCombat().appendVengeance(targetId, damage);
				}
				if (damage > 0)
					c.getCombat().applyRecoil(damage, targetId);
				boolean dropArrows = true;
				if (c.playerEquipment[c.playerWeapon] == 23901 && c.playerEquipment[c.playerWeapon] == 23902 && c.playerEquipment[c.playerWeapon] == 23903 && c.playerEquipment[c.playerWeapon] == 23983) {
				...
				if (c.rangeEndGFX > 0 && !c.getCombat().usingBolts(c.lastArrowUsed)) {
					if (c.rangeEndGFXHeight) {
						target.gfx100(c.rangeEndGFX);
						c.inSpecMode = false;
					} else {
						target.gfx0(c.rangeEndGFX);
						c.inSpecMode = false;
					}
				}
				if (c.dbowSpec) {
					target.gfx100(Arrow.matchesMaterial(c.lastArrowUsed, Arrow.DRAGON) ? 1100 : 1103);
					c.dbowSpec = false;
				}
				if (damage > PlayerHandler.players[targetId].getHealth().getAmount()) {
					damage = PlayerHandler.players[targetId].getHealth().getAmount();
				}
				...
				target.appendDamage(damage, d.getHitmark());
				target.addDamageTaken(c, damage);
				c.killedBy = PlayerHandler.players[targetId].getIndex();
				c.ignoreDefence = false;
				target.updateRequired = true;
				c.getCombat().applySmite(targetId, damage);
				...
				if (c.getCombat().getEndGfxHeight() == 100 && !c.magicFailed) { // end
					target.gfx100(MagicData.MAGIC_SPELLS[c.oldSpellId][5]);
				} else if (!c.magicFailed) {
					target.gfx0(MagicData.MAGIC_SPELLS[c.oldSpellId][5]);
				} else if (c.magicFailed) {
					target.gfx100(85);
				}
				if (!c.magicFailed) {
					if (System.currentTimeMillis() - target.reduceStat > 35000) {
						...
						case 12999:
						case 13023:
							target.getSkills().decreaseLevelOrMin((target.getSkills().getActualLevel(Skill.ATTACK) * 10) / 100, Skill.ATTACK);
							target.getSkills().sendRefresh();
							break;
						}
					}
					...
					case 1153:
						target.getSkills().decreaseLevelOrMin((target.getSkills().getActualLevel(Skill.ATTACK) * 5) / 100, Skill.ATTACK);
						target.sendMessage("Your attack level has been reduced!");
						target.reduceSpellDelay[c.reduceSpellId] = System.currentTimeMillis();
						target.getPA().refreshSkill(0);
						break;

					case 1157:
						target.getSkills().decreaseLevelOrMin((target.getSkills().getActualLevel(Skill.STRENGTH) * 5) / 100, Skill.STRENGTH);
					...
					case 1161:
						target.getSkills().decreaseLevelOrMin((target.getSkills().getActualLevel(Skill.DEFENCE) * 5) / 100, Skill.DEFENCE);
					...
					case 1542:
						target.getSkills().decreaseLevelOrMin((target.getSkills().getActualLevel(Skill.DEFENCE) * 10) / 100, Skill.DEFENCE);
						target.sendMessage("Your defence level has been reduced!");
						target.reduceSpellDelay[c.reduceSpellId] = System.currentTimeMillis();
						target.getPA().refreshSkill(1);
						break;
					...
					case 1543:
						target.getSkills().decreaseLevelOrMin((target.getSkills().getActualLevel(Skill.STRENGTH) * 10) / 100, Skill.STRENGTH);
						target.sendMessage("Your strength level has been reduced!");
						target.reduceSpellDelay[c.reduceSpellId] = System.currentTimeMillis();
						target.getPA().refreshSkill(2);
						break;
				...
				if (tridentOfTheSwampEffect.isExecutable(c)) {
					tridentOfTheSwampEffect.execute(c, NPCHandler.npcs[targetId], new Damage(6));
				}
				...
				c.getCombat().applySmite(targetId, damage);
				c.killedBy = target.getIndex();
				target.updateRequired = true;
				c.usingMagic = false;
				if (target.inMulti() && c.getCombat().multis()) {
					c.barrageCount = 0;
					for (int j = 0; j < PlayerHandler.players.length; j++) {
						if (PlayerHandler.players[j] != null) {
							if (j == target.getIndex())
								continue;
							if (c.barrageCount >= 9)
								break;
							if (target.goodDistance(target.getX(), target.getY(), PlayerHandler.players[j].getX(), PlayerHandler.players[j].getY(), 1))
	...
	public static void calculateCombatDamage(Player attacker, Player defender, CombatType combatType, Special special) {
		...
		if (Objects.nonNull(attacker) && Objects.nonNull(defender)) {
			if (combatType.equals(CombatType.MELEE)) {
				...
				if (defender.playerEquipment[defender.playerShield] == 32991) {
						damage *= .8;
				}
				if (EquipmentSet.TORAG.isWearingBarrows(defender) && defender.getItems().isWearingItem(12853)) {
					defence += defender.getSkills().getActualLevel(Skill.HITPOINTS) - defender.getSkills().getLevel(Skill.HITPOINTS);
				}
				...
				if (attacker.lastWeaponUsed == 22325) { //SCYTHE OF VITUR
					int sHit2 = damage / 2;
					int sHit3 = damage / 4;
				...
				if (attacker.lastWeaponUsed == 33258) { //Tektons sword
					...
					defender.sendMessage("You have been affected by Tekton's Wrath!");
					CycleEventHandler.getSingleton().addEvent(attacker, new CycleEvent() {
						...
						public void execute(CycleEventContainer container) {
							...
							attacker.hitCount++;
							defender.appendDamage(Misc.random(2, 5), Hitmark.HIT);
						}
					}, 2); // handles delay between dmg (600ms = 1 tick)
				}
			}
				
				if ((attacker.lastWeaponUsed == 33139) ||(attacker.lastWeaponUsed == 33140) ||(attacker.lastWeaponUsed == 33141)) { //sara/Bandos/Zammy Bow
					if (Misc.random(1,15) == 1) {
						int maxHit = attacker.getCombat().calculateMeleeMaxHit();
						damage2 = Misc.random(maxHit);
					}
				}
				if (attacker.lastWeaponUsed == 33116) { //zilly Bow
				...
				if (damage > 0 && attacker.lastWeaponUsed == 33562) {
					if (Misc.random(1, 10) == 1) {
						int freezeTime = Misc.random(6, 12);
						defender.freezeTimer = freezeTime;
						attacker.sendMessage("You freeze your opponent for " + freezeTime + " seconds.");
						damage2 = freezeTime;
						defender.gfx100(363);
					}
				}
				...
				hitmark3 = damage3 > 0 ? Hitmark.HIT : Hitmark.MISS;
				if (attacker.getMode().isPVPCombatExperienceGained()) {
					addCombatXP(attacker, CombatType.RANGE, damage + (damage2 > 0 ? damage2 : 0) + (damage3 > 0 ? damage3 : 0));
				}
				...
				if (attacker.fullVoidSupremeMage() && attacker.playerEquipment[attacker.playerWeapon] == 8841) {
					damage = Misc.random(maximumDamage + 18);
				}
				...
				if (MagicData.MAGIC_SPELLS[attacker.oldSpellId][0] == 12445 && !attacker.magicFailed) {
					if (System.currentTimeMillis() - defender.teleBlockDelay > defender.teleBlockLength) {
						...
						if (defender.prayerActive[16] && System.currentTimeMillis() - defender.protMageDelay > 1500 || (defender.summonId == Pets.MINI_JUSTICIAR.getNpcId())) {
				...
				if (freezeDelay > 0 && defender.freezeTimer <= -3 && !attacker.magicFailed) {	
					defender.freezeTimer = (defender.summonId == Pets.MINI_JUSTICIAR.getNpcId() ? freezeDelay / 2 : freezeDelay);
				...
				hitmark3 = damage3 > 0 ? Hitmark.HIT : Hitmark.MISS;
				if (attacker.getMode().isPVPCombatExperienceGained()) {
					addCombatXP(attacker, CombatType.MAGE, damage + (damage2 > 0 ? damage2 : 0) + (damage3 > 0 ? damage3 : 0));
				}
		...
		if (damage3 > -1) {
			attacker.getDamageQueue().add(new Damage(defender, damage3, delay, attacker.playerEquipment, hitmark3, combatType, special));
		}
		delay += 1;
	}
	...
	public static void attackPlayer(Player c, int i) {
		...
		if (c.checkCombatDistance(c, o)) {
			...
			if (PlayerHandler.players[i] != null) {
				...
				/*
				 * Adds player to event list so they get reward at end of event
				 */
				if(c2 == WildernessEscape.host && !WildernessEscape.participants.contains(c) && WildernessEscape.eventActive == true) {
					c.sendMessage("You have attacked the host and have been added officialy to the event");
					WildernessEscape.participants.add(c);
				}
				if (c.playerEquipment[c.playerWeapon] == 9703) {
					c.sendMessage("You cannot attack players with training sword.");
					return;
				}
				if (c.getItems().isWearingAnyItem(4212, 4213, 4214, 4215, 4216, 4217, 4218, 4219, 4220, 4221, 4222, 4223, 33030, 33908, 33909, 33910, 33911, 33914, 33922) && c.playerEquipment[c.playerArrows] > -1) {
				...
				boolean sameSpot = c.getX() == PlayerHandler.players[i].getX() && c.getY() == PlayerHandler.players[i].getY();
				...
				if (PlayerHandler.players[i].getHeight() != c.getHeight()) {
					c.getCombat().resetPlayerAttack();
					return;
				}
				c.followId = i;
				...
				if (c.attackTimer <= 0) {
					...
					c.usingCross = c.playerEquipment[c.playerWeapon] == 4734 || c.playerEquipment[c.playerWeapon] == 21902  || c.playerEquipment[c.playerWeapon] == 9185 ||
								   c.playerEquipment[c.playerWeapon] == 33094 || c.playerEquipment[c.playerWeapon] == 11785 || c.playerEquipment[c.playerWeapon] == 21012 ||
								   c.playerEquipment[c.playerWeapon] == 33117 || c.playerEquipment[c.playerWeapon] == 33114 || c.playerEquipment[c.playerWeapon] == 33124 ||
								   c.playerEquipment[c.playerWeapon] == 33094 || c.playerEquipment[c.playerWeapon] == 33578;
					c.usingBallista = c.playerEquipment[c.playerWeapon] == 19481 || c.playerEquipment[c.playerWeapon] == 19478;
					c.projectileStage = 0;
					CombatType combatType;
					if (c.getX() == PlayerHandler.players[i].getX() && c.getY() == PlayerHandler.players[i].getY()) {
						if (c.freezeTimer > 0) {
							c.getCombat().resetPlayerAttack();
							return;
						}
					...
					if (c.autocasting) {
						...
						if (MagicData.MAGIC_SPELLS[c.spellId][0] >= 12861 && MagicData.MAGIC_SPELLS[c.spellId][0] <= 13023) {
							if (c.playerEquipment[c.playerWeapon] != 4675 && c.playerEquipment[c.playerWeapon] != 6914 && c.playerEquipment[c.playerWeapon] != 21006 && c.playerEquipment[c.playerWeapon] != 22555 && c.playerEquipment[c.playerWeapon] != 22552 && c.playerEquipment[c.playerWeapon] != 12904) {
					...
					if (!c.getCombat().correctBowAndArrows() && Config.CORRECT_ARROWS && c.usingBow && !c.getCombat().usingCrystalBow() && c.playerEquipment[c.playerWeapon] != 4734
							&& c.playerEquipment[c.playerWeapon] != 9185 && c.playerEquipment[c.playerWeapon] != 21902 && c.playerEquipment[c.playerWeapon] != 11785 && c.playerEquipment[c.playerWeapon] != 33094 && c.playerEquipment[c.playerWeapon] != 33578 && c.playerEquipment[c.playerWeapon] != 33124 && c.playerEquipment[c.playerWeapon] != 33094 && c.playerEquipment[c.playerWeapon] != 21012 && c.playerEquipment[c.playerWeapon] != 33117 && c.playerEquipment[c.playerWeapon] != 33114 && !c.usingMagic && !c.getItems().isWearingItem(12926) && c.playerEquipment[c.playerWeapon] != 19478 && c.playerEquipment[c.playerWeapon] != 19481) {
					...
					if (c.playerEquipment[c.playerWeapon] == 9185 && c.playerEquipment[c.playerWeapon] == 33094 && c.playerEquipment[c.playerWeapon] == 21902 && !c.getCombat().properBolts() && !c.usingMagic
							|| c.playerEquipment[c.playerWeapon] == 11785 && c.playerEquipment[c.playerWeapon] == 33094 && c.playerEquipment[c.playerWeapon] == 33124 && !c.getCombat().properBolts() && !c.usingMagic  || c.playerEquipment[c.playerWeapon] == 33578 && !c.getCombat().properBolts() && !c.usingMagic  || c.playerEquipment[c.playerWeapon] == 33117 && !c.getCombat().properBolts() && !c.usingMagic  || c.playerEquipment[c.playerWeapon] == 21012 && !c.getCombat().properBolts() && !c.usingMagic || c.playerEquipment[c.playerWeapon] == 33114 && !c.getCombat().properBolts() && !c.usingMagic) {
					...
					if (projectile) {
						if (!PathChecker.isProjectilePathClear(c.getX(), c.getY(), c.getHeight(), o.getX(), o.getY()) 
								&& !PathChecker.isProjectilePathClear(o.getX(), o.getY(), c.getHeight(), c.getX(), c.getY())) {
							c.attackTimer = 1;
							return;
						} else {
							c.stopMovement();
					...
					if (c.usingMagic) { // magic hit delay
						...
						c.spellId = 0;
						if (MagicData.MAGIC_SPELLS[c.oldSpellId][0] == 12891 && target.getMovementQueue().isMoving()) {
