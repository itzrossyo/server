...
public class ItemAssistant {
	...
	public void sendItemToAnyTabOrDrop(BankItem item, int x, int y) {
		...
		if (ItemUtility.itemIsNote[item.getId()] && bankContains(item.getId() - 2)) {
	 ...
	 */
	public int getWornItemAmount(int itemID, int slot) {
		return slot >= 0 && c.playerEquipment[slot] == itemID ? c.playerEquipmentN[slot] :  0;
	}
	/**
	 * Check all slots and determine the amount of said item worn in that slot
	 */
	...
	public void addItemToBank(int itemId, int amount) {
		...
		if (ItemUtility.itemIsNote[itemId]) {
	...
	public int getTotalCount(int itemID) {
		...
		for (int j = 0; j < c.playerItems.length; j++) {
			if (ItemUtility.itemIsNote[itemID + 1]) {
		...
		for (int i = 0; i < c.playerItems.length; i++) {
			...
			if (killer != null && killer instanceof Player) {
				...
				if (isTradable(c.playerItems[i] - 1) && c.playerItems[i] - 1 != 13307) {
					if (playerKiller.getMode().isItemScavengingPermitted(null, null) || playerKiller.inClanWars()) {
						World.getWorld().getItemHandler().createGroundItem(playerKiller, c.playerItems[i] - 1, c.getX(), c.getY(), c.getHeight(), c.playerItemsN[i], c.killerId);
					} else {
						World.getWorld().getItemHandler().createUnownedGroundItem(c.playerItems[i] - 1, c.getX(), c.getY(), c.getHeight(), c.playerItemsN[i]);
					}
				}
			} else {
				if (!keepItem) {
					World.getWorld().getItemHandler().createGroundItem(c, c.playerItems[i] - 1, c.getX(), c.getY(), c.getHeight(), c.playerItemsN[i], c.getIndex());
				}
			}
			addToDroppedItems(droppedItems, new Item(c.playerItems[i] - 1, c.playerItemsN[i]));
		}
			...
			if (killer != null && killer instanceof Player) {
				...
				if (isTradable(c.playerEquipment[e])) {
					if (playerKiller.getMode().isItemScavengingPermitted(null, null)) {
						World.getWorld().getItemHandler().createGroundItem(playerKiller, c.playerEquipment[e], c.getX(), c.getY(), c.getHeight(), c.playerEquipmentN[e], c.killerId);
					} else {
						World.getWorld().getItemHandler().createUnownedGroundItem(c.playerEquipment[e], c.getX(), c.getY(), c.getHeight(), c.playerEquipmentN[e]);
					}
				}
			} else {
				if (!keepItem) {
					World.getWorld().getItemHandler().createGroundItem(c, c.playerEquipment[e], c.getX(), c.getY(), c.getHeight(), c.playerEquipmentN[e], c.getIndex());
				}
			}
			addToDroppedItems(droppedItems, new Item(c.playerEquipment[e], c.playerEquipmentN[e]));
		}
		if (killer != null) {
			if (killer instanceof Player) {
				...
				World.getWorld().getItemHandler().createGroundItem(playerKiller, 526, c.getX(), c.getY(), c.getHeight(), 1);
			} else if (killer instanceof NPC) {
				NPC npc = (NPC) killer;
				if (Config.SERVER_STATE == ServerState.PUBLIC_PRIMARY) {
					//TODO PvM Log Kills
				}
				World.getWorld().getItemHandler().createGroundItem(c, 526, c.getX(), c.getY(), c.getHeight(), 1);
			}
		}
	}
	 ...
	 * 
	 * @param droppedItems A list of {@link Item}s.
	 * @param item An item which is to be added to the list.
	 */
	private void addToDroppedItems(List<Item> droppedItems, Item item) {
		for (Item listElement : droppedItems) {
	...
	public int getItemCount(int itemId, boolean includeCounterpart) {
		...
		for (int i = 0; i < c.playerItems.length; i++) {
			if (c.playerItems[i] == itemId + 1 || counterpart > 0 && c.playerItems[i] == counterpart + 1) {
				...
				if (ItemUtility.itemStackable[c.playerItems[i] - 1]) {
					break;
				}
			}
		}
		for (int i = 0; i < c.playerEquipment.length; i++) {
			if (c.playerEquipment[i] == itemId || counterpart > 0 && c.playerEquipment[i] == counterpart) {
				...
				if (ItemUtility.itemStackable[c.playerEquipment[i] - 1]) {
					break;
				}
			}
		}
	...
	public boolean isTradable(int itemId) {
		...
		if (ItemUtility.getItemName(itemId).contains("graceful")) {
			return false;
		}
	...
	public boolean addItem(int item, int amount) {
		...
		if ((((freeSlots() >= 1) || playerHasItem(item, 1)) && ItemUtility.itemStackable[item]) || ((freeSlots() > 0) && !ItemUtility.itemStackable[item])) {
			for (int i = 0; i < c.playerItems.length; i++) {
				if ((c.playerItems[i] == (item + 1)) && ItemUtility.itemStackable[item] && (c.playerItems[i] > 0)) {
	...
	public String itemType(int item) {
		for (int i = 0; i < ItemUtility.capes.length; i++) {
			if (item == ItemUtility.capes[i])
				return "cape";
		}
		for (int i = 0; i < ItemUtility.hats.length; i++) {
			if (item == ItemUtility.hats[i])
				return "hat";
		}
		for (int i = 0; i < ItemUtility.boots.length; i++) {
			if (item == ItemUtility.boots[i])
				return "boots";
		}
		for (int i = 0; i < ItemUtility.gloves.length; i++) {
			if (item == ItemUtility.gloves[i])
				return "gloves";
		}
		for (int i = 0; i < ItemUtility.shields.length; i++) {
			if (item == ItemUtility.shields[i])
				return "shield";
		}
		for (int i = 0; i < ItemUtility.amulets.length; i++) {
			if (item == ItemUtility.amulets[i])
				return "amulet";
		}
		for (int i = 0; i < ItemUtility.arrows.length; i++) {
			if (item == ItemUtility.arrows[i])
				return "arrows";
		}
		for (int i = 0; i < ItemUtility.rings.length; i++) {
			if (item == ItemUtility.rings[i])
				return "ring";
		}
		for (int i = 0; i < ItemUtility.body.length; i++) {
			if (item == ItemUtility.body[i])
				return "body";
		}
		for (int i = 0; i < ItemUtility.legs.length; i++) {
			if (item == ItemUtility.legs[i])
				return "legs";
		}
		return "weapon";
	}
	...
	public void getBonus() {
		for (int i = 0; i < c.playerEquipment.length; i++) {
			if (c.playerEquipment[i] > -1) {
				...
				ItemList itemList = World.getWorld().getItemHandler().ItemList[equipmentItem];
				if (itemList == null) {
					continue;
				}
	...
	public void sendWeapon(int Weapon, String WeaponName) {
		...
		} else if (WeaponName.equals("Staff of the dead") || WeaponName.equals("Toxic staff of the dead") || c.playerEquipment[c.playerWeapon] == 33277 || WeaponName.equalsIgnoreCase("dawnbringer")) {
		...
		} else if (WeaponName2.contains("warhammer") || c.playerEquipment[c.playerWeapon] == 33282 || c.playerEquipment[c.playerWeapon] == 4153 || c.playerEquipment[c.playerWeapon] == 13263 || c.playerEquipment[c.playerWeapon] == 12848 || c.playerEquipment[c.playerWeapon] == 13902) {
		...
		if (c.playerItems[slot] == (wearID + 1)) {
			...
			case 20796:
				if(!c.getMode().isHcIronman()) {
					c.sendMessage("You are not a Hardcore Ironman!");
					return false;
				}				
				break;
			
				case 21003:
					if(c.getSkills().getActualLevel(Skill.ATTACK) < 75) {
						c.sendMessage("You must have a attack level of at least 75 to weild the Elder Maul");
						return false;
					}
					if(c.getSkills().getActualLevel(Skill.STRENGTH) < 75) {
						c.sendMessage("You must have a strength level of at least 75 to wield the Elder Maul");
						return false;
					}
					break;
				...
				case 20784:
					if(c.getSkills().getActualLevel(Skill.ATTACK) < 60) {
				...
				case 13197:
					...
					break;
				case 21633:
					if(c.getSkills().getActualLevel(Skill.MAGIC) < 70) {
				...
				case 21301:
				case 21304:
					if(c.getSkills().getActualLevel(Skill.DEFENCE) < 60) {
				...
				case 19481:
					...
					break;
				case 19478:
					if(c.getSkills().getActualLevel(Skill.RANGED) < 65) {
			...
			case 20760:
				...
				//master capes exp required
			case 33033:
				if (c.getSkills().getExperience(Skill.AGILITY) < 199999999) { 
					c.sendMessage("you need 200,000,000 (200M) Exp to wear this cape.");
					return false;
				}
				break;
			case 33034:
				if (c.getSkills().getExperience(Skill.ATTACK) < 199999999) {
					c.sendMessage("you need 200,000,000 (200M) Exp to wear this cape.");
					return false;
				}
				break;
			case 33035:
				if (c.getSkills().getExperience(Skill.CONSTRUCTION) < 199999999) {
					c.sendMessage("you need 200,000,000 (200M) Exp to wear this cape.");
					return false;
				}
				break;
			case 33036:
				if (c.getSkills().getExperience(Skill.COOKING) < 199999999) {
					c.sendMessage("you need 200,000,000 (200M) Exp to wear this cape.");
					return false;
				}
				break;
			case 33037:
				if (c.getSkills().getExperience(Skill.CRAFTING) < 200000000) {
					c.sendMessage("you need 200,000,000 (200M) Exp to wear this cape.");
					return false;
				}
				break;
			case 33038:
				if (c.getSkills().getExperience(Skill.DEFENCE) < 199999999) {
					c.sendMessage("you need 200,000,000 (200M) Exp to wear this cape.");
					return false;
				}
				break;
			case 33039:
				if (c.getSkills().getExperience(Skill.FARMING) < 199999999) {
					c.sendMessage("you need 200,000,000 (200M) Exp to wear this cape.");
					return false;
				}
				break;
			case 33040:
				if (c.getSkills().getExperience(Skill.FIREMAKING) < 199999999) {
					c.sendMessage("you need 200,000,000 (200M) Exp to wear this cape.");
					return false;
				}
				break;
			case 33041:
				if (c.getSkills().getExperience(Skill.FISHING) < 199999999) {
					c.sendMessage("you need 200,000,000 (200M) Exp to wear this cape.");
					return false;
				}
				break;
			case 33042:
				if (c.getSkills().getExperience(Skill.FLETCHING) < 199999999) {
					c.sendMessage("you need 200,000,000 (200M) Exp to wear this cape.");
					return false;
				}
				break;
			case 33043:
				if (c.getSkills().getExperience(Skill.HERBLORE) < 199999999) {
					c.sendMessage("you need 200,000,000 (200M) Exp to wear this cape.");
					return false;
				}
				break;
			case 33044:
				if (c.getSkills().getExperience(Skill.HITPOINTS) < 199999999) {
					c.sendMessage("you need 200,000,000 (200M) Exp to wear this cape.");
					return false;
				}
				break;
			case 33045:
				if (c.getSkills().getExperience(Skill.HUNTER) < 199999999) {
					c.sendMessage("you need 200,000,000 (200M) Exp to wear this cape.");
					return false;
				}
				break;
			case 33046:
				if (c.getSkills().getExperience(Skill.MAGIC) < 199999999) {
					c.sendMessage("you need 200,000,000 (200M) Exp to wear this cape.");
					return false;
				}
				break;
			case 33047:
				if (c.getSkills().getExperience(Skill.MINING) < 199999999) {
					c.sendMessage("you need 200,000,000 (200M) Exp to wear this cape.");
					return false;
				}
				break;
			case 33048:
				if (c.getSkills().getExperience(Skill.PRAYER) < 199999999) {
					c.sendMessage("you need 200,000,000 (200M) Exp to wear this cape.");
					return false;
				}
				break;
			case 33049:
				if (c.getSkills().getExperience(Skill.RANGED) < 199999999) {
					c.sendMessage("you need 200,000,000 (200M) Exp to wear this cape.");
					return false;
				}
				break;
			case 33050:
				if (c.getSkills().getExperience(Skill.RUNECRAFTING) < 199999999) {
					c.sendMessage("you need 200,000,000 (200M) Exp to wear this cape.");
					return false;
				}
				break;
			case 33051:
				if (c.getSkills().getExperience(Skill.SLAYER) < 199999999) {
					c.sendMessage("you need 200,000,000 (200M) Exp to wear this cape.");
					return false;
				}
				break;
			case 33052:
				if (c.getSkills().getExperience(Skill.SMITHING) < 199999999) {
					c.sendMessage("you need 200,000,000 (200M) Exp to wear this cape.");
					return false;
				}
				break;
			case 33053:
				if (c.getSkills().getExperience(Skill.STRENGTH) < 199999999) {
					c.sendMessage("you need 200,000,000 (200M) Exp to wear this cape.");
					return false;
				}
				break;
			case 33054:
				if (c.getSkills().getExperience(Skill.THIEVING) < 199999999) {
					c.sendMessage("you need 200,000,000 (200M) Exp to wear this cape.");
					return false;
				}
				break;
			case 33055:
				if (c.getSkills().getExperience(Skill.WOODCUTTING) < 199999999) {
					c.sendMessage("you need 200,000,000 (200M) Exp to wear this cape.");
					return false;
				}
				break;
			...
			for (int skillId = 0; skillId < item.getRequirements().length; skillId++) {
				if(skillId >= Skill.values().length) {
					System.out.println("Item " + item.getId() + " has incorrect skill requirements!");
					continue;
				}
			...
			if (slot >= 0 && wearID >= 0) {
				...
				if (toEquip == toRemove + 1 && ItemUtility.itemStackable[toRemove]) {
		...
		if (c.getOutStream() != null && c != null) {
			if (c.playerEquipment[slot] > -1) {
				if (addItem(c.playerEquipment[slot], c.playerEquipmentN[slot])) {
					...
					c.getCombat().getPlayerAnimIndex(getItemName(c.playerEquipment[c.playerWeapon]).toLowerCase());
					c.getOutStream().writePacketHeader(34);
	...
	public void removeFromBank(int itemId, int itemAmount, boolean updateView) {
		...
		if (c.takeAsNote) {
			...
			if (!ItemUtility.itemIsNote[itemId] && World.getWorld().getItemHandler().getCounterpart(itemId) > 0) {
				noted = true;
			} else
		...
		if (item.getAmount() == 0) {
			if (!c.placeHolderWarning) {
				...
				c.sendMessage("@cr10@@red@Are you sure you want to release the placeholder of " + ItemUtility.getItemName(item.getId() - 1) + "?");
		...
		if (!noted)
			...
			addItem(World.getWorld().getItemHandler().getCounterpart(item.getId() - 1), item.getAmount());
	...
	public boolean isStackable(int itemID) {
		return ItemUtility.itemStackable[itemID];
	}
	...
	public void deleteArrow() {
		if (c.getItems().isWearingItem(10499, c.playerCape) || SkillcapePerks.isWearingMasterCape(c) || c.getItems().isWearingItem(22109, c.playerCape) ||SkillcapePerks.RANGING.isWearing(c) || SkillcapePerks.isWearingMasterCape(c) || SkillcapePerks.isWearingMaxCape(c) && !c.getItems().isWearingItem(4734) &&
				!c.getItems().isWearingItem(10033) && !c.getItems().isWearingItem(10034) && !c.getItems().isWearingItem(11959) && !c.getItems().isWearingItem(33466)) {
	...
	public void removeRangedWeaponry() {
		if (c.getItems().isWearingItem(10499, c.playerCape) || SkillcapePerks.RANGING.isWearing(c) || SkillcapePerks.isWearingMasterCape(c) || SkillcapePerks.isWearingMaxCape(c) && !c.getItems().isWearingItem(4734) && 
				!c.getItems().isWearingItem(10033) && !c.getItems().isWearingItem(10034) && !c.getItems().isWearingItem(11959) && !c.getItems().isWearingItem(33466)) {
			if (RandomUtils.nextInt(0, 15) > 1) {
				return;
			}
	...
	public void deleteEquipment() {
		if (!c.getItems().isWearingItem(10033) && !c.getItems().isWearingItem(10034)
				&& !c.getItems().isWearingItem(11959) && !c.getItems().isWearingItem(33466)) {
			if (c.getItems().isWearingItem(10499, c.playerCape) || c.getItems().isWearingItem(22109, c.playerCape) || SkillcapePerks.isWearingMasterCape(c) || SkillcapePerks.RANGING.isWearing(c)
	...
	public void dropArrowNpc(NPC npc) {
		if (c.playerEquipment[c.playerCape] == 10499 || c.getItems().isWearingItem(22109, c.playerCape) || SkillcapePerks.isWearingMasterCape(c) ||SkillcapePerks.RANGING.isWearing(c) || SkillcapePerks.isWearingMaxCape(c)) {
			return;
		}
		int enemyX = npc.getX();
		int enemyY = npc.getY();
		...
		if (Misc.random(10) >= 4) {
			if (World.getWorld().getItemHandler().itemAmount(c.playerName, c.rangeItemUsed, enemyX, enemyY, height) == 0) {
				World.getWorld().getItemHandler().createGroundItem(c, c.rangeItemUsed, enemyX, enemyY, height, 1, c.getIndex());
			} else if (World.getWorld().getItemHandler().itemAmount(c.playerName, c.rangeItemUsed, enemyX, enemyY, height) != 0) {
				int amount = World.getWorld().getItemHandler().itemAmount(c.playerName, c.rangeItemUsed, enemyX, enemyY, height);
				World.getWorld().getItemHandler().removeGroundItem(c, c.rangeItemUsed, enemyX, enemyY, height, false);
				World.getWorld().getItemHandler().createGroundItem(c, c.rangeItemUsed, enemyX, enemyY, height, amount + 1, c.getIndex());
			}
		}
	}
	...
	public void dropArrowPlayer() {
		...
		int height = PlayerHandler.players[c.oldPlayerIndex].getHeight();
		if (c.playerEquipment[c.playerCape] == 10499 || c.getItems().isWearingItem(22109, c.playerCape) || SkillcapePerks.isWearingMasterCape(c) ||SkillcapePerks.RANGING.isWearing(c) || SkillcapePerks.isWearingMaxCape(c))
			return;
		if (Misc.random(10) >= 4) {
			if (World.getWorld().getItemHandler().itemAmount(c.playerName, c.rangeItemUsed, enemyX, enemyY, height) == 0) {
	...
	public static String getItemName(int itemId) {
		if (itemId < 0) {
			return "Unarmed (" + itemId + ")";
		}
		ItemList itemList = World.getWorld().getItemHandler().ItemList[itemId];
		if (itemList == null) {
			return "Unarmed (" + itemId + ")";
		}
		return itemList.itemName == null ? "Unarmed (" + itemId + ")" : itemList.itemName;
	}
	...
	public int getItemId(String itemName) {
		for (int i = 0; i < Config.ITEM_LIMIT; i++) {
			if (World.getWorld().getItemHandler().ItemList[i] != null) {
				if (World.getWorld().getItemHandler().ItemList[i].itemName.equalsIgnoreCase(itemName)) {
	...
	public int getItemAmount(int itemId, int slot) {
		return slot >= 0 && c.playerItems[slot] - 1 == itemId ? c.playerItemsN[slot] : 0;
	}
	...
	public int getUnnotedItem(int ItemID) {
		...
		for (int i = 0; i < Config.ITEM_LIMIT; i++) {
			if (World.getWorld().getItemHandler().ItemList[i] != null) {
				if (Objects.equals(World.getWorld().getItemHandler().ItemList[i].itemName, NotedName)) {
					if (!World.getWorld().getItemHandler().ItemList[i].itemDescription.startsWith("Swap this note at any bank for a")) {
	...
	public void createGroundItem(int itemID, int itemX, int itemY, int itemAmount) {
		if (c.getOutStream() != null) {
			...
			c.getOutStream().writePacketHeader(44);
	...
	public void removeGroundItem(int itemID, int itemX, int itemY, int Amount) {
		...
		c.getOutStream().writeByteC((itemY - 8 * c.getLastKnownLocation().getRegionY()));
		c.getOutStream().writeByteC((itemX - 8 * c.getLastKnownLocation().getRegionX()));
		c.getOutStream().writePacketHeader(156);
	...
	public void createGroundItem(GroundItem item) {
		c.getOutStream().writePacketHeader(85);
		c.getOutStream().writeByteC((item.getY() - 8 * c.getLastKnownLocation().getRegionY()));
		c.getOutStream().writeByteC((item.getX() - 8 * c.getLastKnownLocation().getRegionX()));
		c.getOutStream().writePacketHeader(44);
		c.getOutStream().writeWordBigEndianA(item.getId());
	...
	public void handleAmethyst() {
		if (!c.boltTips&&!c.arrowTips&&!c.javelinHeads) {
			...
			if (c.getSkills().getLevel(Skill.CRAFTING) < 83) {
			...
			if (c.getSkills().getLevel(Skill.CRAFTING) < 85) {
			...
			if (c.getSkills().getLevel(Skill.CRAFTING) < 87) {
	...
	public int getTotalRiskedWorth() {
		...
		resetKeepItems();
		if (!c.isSkulled) {
			int keptItemsCount = c.summonId == Pets.MINI_DERWEN.getNpcId() ? 4 : 3;
			for (int i = 0; i < keptItemsCount; i++) {
				keepItem(i, false);
			}
		}
		if (c.prayerActive[10] && System.currentTimeMillis() - c.lastProtItem > 700) {
			keepItem(3, false);
			if (c.summonId == Pets.MINI_DERWEN.getNpcId()) {
				keepItem(4, false);
			}
		}
		for (int i = 0; i < c.itemKeptId.length; i++) {
	...
	public boolean wearingAny(int itemId) {
		// TODO Auto-generated method stub
		return getWornItemSlot(itemId) > -1;
	}
	...
	public Item getItemAtSlot(int slot) {
		return new Item(c.playerItems[slot] - 1, c.playerItemsN[slot]);
	}
	...
	public boolean containsItems(int... ids) {
		return Stream.of(ids).flatMapToInt(Arrays::stream).allMatch(id -> playerHasItem(id));	
	}
	
	/**
	 * Checks if the player has the items.
	 */
	public boolean containsItems(int[][]... items) {
		return Stream.of(items).flatMap(Stream::of).allMatch(item -> hasItemOrEquipped(item[0], item[1]));	
	}
	
	public boolean isNaked() {
		return this.freeEquipmentSlots() >= c.playerEquipment.length && this.freeSlots() >= c.playerItems.length;
	}

	/**
	 * Checks if the player has any of the ids given.
	 */
	public boolean hasAnyItems() {
		return Stream.of(c.playerEquipment, c.playerItems).flatMapToInt(Arrays::stream).anyMatch(id -> id > 0);	
	}
	
	/*public boolean containsItems(int[][]... items) {
		List<Item> playerItems = getAllItems().collect(Collectors.toList());
		return Stream
				.of(items)
				.flatMap(Arrays::stream)
				.map(value -> new Item(value[0], value[1]))
				.allMatch(item -> playerItems
									.stream()
									.filter(plrItem -> plrItem.getId() == item.getId() && plrItem.getAmount() >= item.getAmount())
									.count() > 0
							);
	}*/
	...
	public Stream<Item> getAllItems(){
		return Streams
				.concat(
					IntStream.range(0, c.playerEquipment.length).filter(slot -> c.playerEquipment[slot] > 0).mapToObj(slot -> new Item(c.playerEquipment[slot], c.playerEquipmentN[slot])),
					IntStream.range(0, c.playerItems.length).filter(slot -> c.playerItems[slot] > 0).mapToObj(slot -> new Item(c.playerItems[slot], c.playerItemsN[slot]))
				);
	}
	
	public boolean hasItemOrEquipped(int toFind) {
		return IntStream.of(c.playerEquipment).anyMatch(id -> id == toFind) || IntStream.of(c.playerItems).anyMatch(id -> id - 1 == toFind);
	}
	
	public boolean hasItemOrEquipped(int id, int amount) {
		for (int slot = 0; slot < c.playerEquipment.length; slot++) {
			int equipId = c.playerEquipment[slot];
			int amt = c.playerEquipmentN[slot];
			if (id == equipId && amt >= amount) {
				return true;
			}
		}
		return playerHasItem(id, amount);
	}
	
	/**
	 * Checks if the player has any of the ids given.
	 */
	public boolean containsAnyItem(int... ids) {
		return Stream.of(ids).flatMapToInt(Arrays::stream).anyMatch(id -> playerHasItem(id));	
	}
	
	public int removeItemFromEquipment(int id, int amount) {
		return IntStream.range(0, c.playerEquipment.length).filter(slot -> {
			int itemAtSlot = c.playerEquipment[slot];
			if (itemAtSlot == id) {
				int amt = c.playerEquipmentN[slot];
				int reduced = amt - amount;
				wearItem(reduced <= 0 ? -1 : id, reduced, slot);
				return true;
			}
			return false;
		}).findFirst().orElse(-1);
	...
	public boolean isWearingItems(int... items) {
		IntStream equipment = IntStream.of(c.playerEquipment);
		for (int item : items) {
			boolean exists = equipment.anyMatch(id -> id == item);
			if (!exists) {
				return false;
			}
		}
