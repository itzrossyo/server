		...
		CycleEventHandler.getSingleton().addEvent(attacker, new CycleEvent() {
			...
			public void execute(CycleEventContainer eventContainer) {
				...
				if (cycle == 1) {
					...
					if (defender.playerEquipment[defender.playerShield] == 33115) {
						defender.appendDamage((damage.getAmount() / 3) + (Misc.random(damage.getAmount() / 3)), damage.getAmount() > 0 ? Hitmark.HIT : Hitmark.MISS);
						eventContainer.stop();
						return;
					}
