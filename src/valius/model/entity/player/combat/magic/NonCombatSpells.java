...
public class NonCombatSpells extends MagicRequirements {
	...
	public static void attemptDate(Player c, int action) {
		...
		case 8014:
			...
			bonesToBase(c, 15, new int[] { 8014, -1, -1 }, new int[] { 1, -1, -1 }, new int[] { !c.getItems().playerHasItem(530) ? 528 : 530, 1963 });
			bonesToBase(c, 15, new int[] { 8014, -1, -1 }, new int[] { 1, -1, -1 }, new int[] { !c.getItems().playerHasItem(536) ? 534 : 536, 1963 });
			break;
		case 8015:
			bonesToBase(c, 60, new int[] { 8015, -1, -1 }, new int[] { 1, -1, -1 }, new int[] { !c.getItems().playerHasItem(532) ? 526 : 532, 6883 });
			bonesToBase(c, 60, new int[] { 8015, -1, -1 }, new int[] { 1, -1, -1 }, new int[] { !c.getItems().playerHasItem(530) ? 528 : 530, 6883 });
			bonesToBase(c, 60, new int[] { 8015, -1, -1 }, new int[] { 1, -1, -1 }, new int[] { !c.getItems().playerHasItem(536) ? 534 : 536, 6883 });
			break;
		}
	}
	...
	public static void superHeatItem(Player c, int itemID) {
		if (System.currentTimeMillis() - c.alchDelay > 700) {
			if (c.getSkills().getLevel(Skill.MAGIC) < 43) {
			...
			for (int i = 0; i < data.length; i++) {
				if (itemID == data[i][0]) {
					...
					if (c.getSkills().getLevel(Skill.SMITHING) < data[i][6]) {
					...
					c.getPA().addSkillXP(data[i][5] * 1, Player.playerSmithing, true);
					c.getPA().addSkillXP(53 * 1, 6, true);
			...
			if (System.currentTimeMillis() - c.alchDelay > 1000) {
				if (c.getItems().playerHasItem(itemId, slot, 1)) {
					...
					if (c.getItems().playerHasItem(itemId, slot, 1)) {
						...
						c.alchDelay = System.currentTimeMillis();
						c.getPA().sendFrame106(6);
						c.getPA().addSkillXP(MagicData.MAGIC_SPELLS[49][7] * 1, 6, true);
			...
			if (System.currentTimeMillis() - c.alchDelay > 2000) {
				if (c.getItems().playerHasItem(itemId, slot, 1)) {
					...
					if (c.getItems().playerHasItem(itemId, slot, 1)) {
						...
						c.alchDelay = System.currentTimeMillis();
						c.getPA().sendFrame106(6);
						c.getPA().addSkillXP(MagicData.MAGIC_SPELLS[50][7] * 1, 6, true);
						c.getPA().refreshSkill(6);
					}
				}
			}
