package valius.model.minigames.warriors_guild;
...
public class AnimatedArmour {
	...
	public static void itemOnAnimator(final Player player, int itemId) {
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer event) {
				World.getWorld().getNpcHandler().spawnNpc(player, armour.getNpcId(), animator_west ? SPAWN_X : 2857, SPAWN_Y, 0, 1, armour.getHealth(), armour.getMax(), armour.getAttack(), armour.getDefence(), true,
						true);
	...
	public static void dropTokens(Player player, int npcType, int x, int y) {
		...
		if (npc != null) {
				World.getWorld().getItemHandler().createGroundItem(player, 8851, x, y, player.getHeight(), npc.getAmountOfTokens(), player.getIndex());
