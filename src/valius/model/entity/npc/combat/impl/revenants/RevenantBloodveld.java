...
@ScriptSettings(npcIds = { 3374 })

public class RevenantBloodveld extends CombatScript {
	...
	public int attack(NPC npc, Entity target) {
		...
		if (target.asPlayer().protectingMelee()) {
			switch (randomAttack) {
			case 1:
				handleHit(npc, target, CombatType.MAGE, MAGE_PROJECTILE, new Hit(Hitmark.HIT, getRandomMaxHit(npc, target, CombatType.MAGE, 32), 3));
				break;
			case 2:
				handleHit(npc, target, CombatType.RANGE, RANGE_PROJECTILE, new Hit(Hitmark.HIT, getRandomMaxHit(npc, target, CombatType.RANGE, 32), 3));
				break;
			}
		} 
		 ...
		 */
			...
			switch (randomAttack) {
			case 1:
				handleHit(npc, target, CombatType.MELEE, new Hit(Hitmark.HIT, getRandomMaxHit(npc, target, CombatType.MELEE, 32), 1));
				break;
			case 2:
