...
import valius.util.MapTileUtils;
import valius.util.Misc;
import valius.util.Utilities;
...
public class Region {
	...
	private static final WorldObject[] EXISTANT_OBJECTS = {
			...
			new WorldObject(29787, 3088, 3488, 0, 0),
			// Abyss
			...
			new WorldObject(14888, 3030, 3381, 0, 3), new WorldObject(25824, 3031, 3381, 0, 1),
			...
			//Mining
			new WorldObject(11377, 2824, 3003, 0, 0), //Rune
			new WorldObject(11377, 2823, 3002, 0, 0), //Rune
			new WorldObject(11377, 2822, 3001, 0, 0), //Rune
			new WorldObject(11377, 2821, 3000, 0, 0), //Rune
			new WorldObject(11377, 2820, 2999, 0, 0), //Rune
			new WorldObject(11377, 2819, 2998, 0, 0), //Rune
			...
			//Woodcutting
			// Magic trees
			new WorldObject(10834, 2853, 2980, 0, 0),
			new WorldObject(10834, 2853, 2983, 0, 0),
			new WorldObject(10834, 2853, 2986, 0, 0),
			new WorldObject(10834, 2853, 2989, 0, 0),
			// Yew trees
			new WorldObject(10822, 2842, 2980, 0, 0),
			new WorldObject(10822, 2842, 2983, 0, 0),
			new WorldObject(10822, 2842, 2986, 0, 0),
			new WorldObject(10822, 2842, 2989, 0, 0),
	...
	public static void addGlobalObject(GlobalObject globalObject) {
		...
		if (worldObjects.containsKey(regionId)) {
			if(!worldObjects.get(regionId).contains(toWorldObject))
				worldObjects.get(regionId).add(toWorldObject);
		} else {
			ArrayList<WorldObject> object = new ArrayList<>(1);
			object.add(toWorldObject);
			worldObjects.put(regionId, object);
		}
	}
	...
	private int getClip(int x, int y, int height) {
		...
		if(height > 3) {
			if (clips.get(height) != null) {
				int flag = clips.get(height)[x - regionAbsX][y - regionAbsY];
				if(clips.get(height & 3) != null)
					flag |= clips.get(height & 3)[x - regionAbsX][y - regionAbsY];
				return flag;
			} else {
				height &= 3;
			}
	...
	public static boolean lineOfSight(Entity entity, Entity other) {
		...
		if (Region.canMove(entity.getX(), entity.getY(), entity.getHeight(), dir)) {
			return true;
		}
	...
	public static int getClipping(int x, int y, int height) {
		return getRegion(x, y).getClip(x, y, height);
	...
	public static void load() {
		try {
			...
			ForkJoinPool.commonPool().submit(
			          () -> MapIndexLoader.stream().parallel().forEach(Region::loadMap) 
			    ).get();
				...
				do {
					...
					if (height == tileZ && localX >= tileX && localX < tileX + 8 && localY >= tileY && localY < tileY + 8) {
						...
						if (locX > 0 && locY > 0 && locX < tileFlags[0].length && locY < tileFlags[0].length) {
							...
							int l4 = height;
							if ((tileFlags[1][locX][locY] & 2) == 2)
								l4--;
							//if(objectId == 36101 || objectId == 36102)
							//log.info("Spawning object {} at {} {} {}", objectId, baseLocation.getX() + locX, baseLocation.getY() + locY, l4);

							addObject(objectId, baseLocation.getX() + locX, baseLocation.getY() + locY, l4, type, orientation + rotation & 3);
							addWorldObject(objectId, baseLocation.getX() + locX, baseLocation.getY() + locY, l4, type, orientation + rotation & 3);
						}
					}
				} while (true);
			} while (true);
		...
		for(int x = 0;x<palette.getWidth();x++) {
			for(int y = 0;y<palette.getLength();y++) {
					...
					if(tile != null) {
						...
						if(data.isPresent()) {
							ByteStream lsData = MapIndexLoader.getLandscapeData(data);
							ByteStream objData = MapIndexLoader.getObjectData(data);
									decodeConstructedMapsLandscape(tileFlags, lsData, tileZ, rotation, x * 8, (tileX & 7) * 8, (tileY & 7) * 8, z, y * 8);
							decodeConstructedMapObjects(minimum, tileFlags, tileZ, x * 8, (tileY & 7) * 8, z, objData, (tileX & 7) * 8, rotation, y * 8);
						}
					}
				}
			}
			...
			for(int x = 0;x<tileFlags[z].length;x++) {
				for(int y = 0;y<tileFlags[z][x].length;y++) {
					if ((tileFlags[z][x][y] & 1) == 1) {
						...
						if (height >= 0 && height <= 3) {
							addClipping(minimum.getX() + x, minimum.getY() + y, height, 0x200000);
						}
					}
				}
			}
