...
public class Inferno extends Tzkalzuk {
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer event) {
				...
				for(int i = 0; i < getKillsRemaining(); i++) {
					...
					World.getWorld().getNpcHandler().spawnNpc(player, npcType, startX, startY, height, 1, hp, maxhit, atk, def, true, false);
				}
				event.stop();
			}
	...
	public void handleDeath() {
		player.getPA().movePlayer(2495, 5110, 0);
		player.getDH().sendStatement("Unfortunately you died on wave " + (player.infernoWaveId + 1) + ". Better luck next time.");
		player.nextChat = 0;
		player.infernoWaveType = 0;
		player.infernoWaveId = 0;
		killAllSpawns();
	...
	public void gamble() {
		...
		if (Misc.random(200) == 0) {
			 if (player.getItems().getItemCount(13225, true) == 0 && player.summonId != 13225) {
				 GlobalMessages.send("" + player.playerName + "</col> received a pet from <col=255>TzTok-Jad</col>", GlobalMessages.MessageType.LOOT);
