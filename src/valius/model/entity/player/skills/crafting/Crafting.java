...
public class Crafting {
	...
	public void cutGem(int id) {
		for (int j = 0; j < gems.length; j++) {
			if (gems[j][0] == id) {
				if (c.getSkills().getLevel(Skill.CRAFTING) >= gems[j][2]) {
					c.getItems().deleteItem(id, c.getItems().getItemSlot(id), 1);
					c.getItems().addItem(gems[j][1], 1);
					c.getPA().addSkillXP(gems[j][3], Player.playerCrafting, true);
					if (Misc.random(40) == 0) {
	...
	public void checkRequirements() {
		for (int j = 0; j < expsAndLevels.length; j++) {
			if (expsAndLevels[j][0] == hideType) {
				if (c.getSkills().getLevel(Skill.CRAFTING) >= expsAndLevels[j][1]) {
	...
	public void craftHides(int id) {
		for (int j = 0; j < amount; j++) {
			...
			c.getPA().addSkillXP(exp , Player.playerCrafting, true);
			if (Misc.random(20) == 0) {
				int sPoints = Misc.random(1, 5);
                c.skillPoints += sPoints;
                c.sendMessage("@pur@You receive " + sPoints + " Skill Points.");
	...
	public void cut(int use, int used) {
		...
		gem.ifPresent(g -> {
			...
			if (c.getSkills().getLevel(Skill.CRAFTING) < g.level) {
				c.sendMessage("You need a crafting level of " + g.level + " to do this.");
				return;
			}
			c.startAnimation(886);
			World.getWorld().getEventHandler().submit(new Event<Player>("skilling", c, 2) {
				...
				public void execute() {
					...
					if (attachment.getItems().playerHasItem(g.uncut)) {
						...
						attachment.getPA().addSkillXP(g.experience, Skill.CRAFTING.getId(), true);
