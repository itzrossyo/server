...
import valius.util.Misc.Direction;
...
public class HalloweenSearchGame {
	 ...
	 */
	public void update() {
		World.getWorld().getGlobalObjects().remove(chest);
		pumpkinsReceived.clear();
		remaining = DEFAULT_PUMPKINS;
		generateLocation();
		updateGhosts();
		World.getWorld().getGlobalObjects().add(chest = new GlobalObject(CHEST_ID, xLocation, yLocation, 0, 2, 10, -1, Integer.MAX_VALUE));
		int index = 0;
		...
		GlobalMessages.send("The ghosts have moved to location @blu@#" + index + "@bla@.", GlobalMessages.MessageType.EVENT);
	}
	...
	public void updateZombies() {
		...
		case 0:
			...
			GlobalMessages.send("The zombies are invading the edgeville general store.", GlobalMessages.MessageType.EVENT);
			break;
		case 1:
			...
			GlobalMessages.send("The zombies are invading the edgeville wilderness ditch.", GlobalMessages.MessageType.EVENT);
			break;
		case 2:
			...
			GlobalMessages.send("The zombies are invading the edgeville dungeon.", GlobalMessages.MessageType.EVENT);
			break;
		}
	}
		...
		if (spawned) {
			for (int[] ghost : GHOST_TYPES) {
				...
				if (Objects.isNull(npc)) {
					...
					npc.actionTimer = 0;
					npc.setX(xLocation + ghost[1]);
					npc.setY(yLocation + ghost[2]);
