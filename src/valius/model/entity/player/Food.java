...
public class Food {
	...
	public static enum FoodToEat {
		...
		PURPLE_SWEETS(10476, 15, "Purple Sweets", 0, "Food", false), 
		...
		PADDLEFISH(23874, 20, "Pattlefish", 0, "Food", false);
	...
	public void foodEffect(int id) {
		...
		case 13441:
			...
			break;
		case 1907:
			c.getSkills().increaseLevel(c.getSkills().getActualLevel(Skill.MAGIC) >= 50 ? 3 : 2, Skill.MAGIC);
			c.getSkills().decreaseLevelOrMin(3, Skill.ATTACK);
			c.getSkills().decreaseLevelOrMin(4, Skill.STRENGTH, Skill.DEFENCE);
	...
	public boolean getLevelBetween(int min, int max) {
		int hitpointsLvl = c.getSkills().getLevel(Skill.HITPOINTS);
		return hitpointsLvl >= min && hitpointsLvl <= max;
	}
