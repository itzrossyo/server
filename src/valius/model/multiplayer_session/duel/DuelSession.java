...
public class DuelSession extends MultiplayerSession {
	...
	public void give() {
		...
		if (!Objects.equals(getOther(winner.get()), winner.get())) {
			...
			if (items.get(winner.get()).size() > 0) {
				for (Item item : items.get(winner.get())) {
		...
		World.getWorld().getItemHandler().reloadItems(winner.get());
		items.clear();
	}
	...
	public void showRewardComponent(Player c) {
		...
		List<Item> itemList = items.get(getOther(c));
		if (itemList.size() > 28) {
		...
		for (Item item : itemList) {
			...
			if (item.getId() > Config.ITEM_LIMIT || item.getId() < 0) {
				item = new Item(Config.ITEM_LIMIT, item.getId());
			}
	...
	class AttackingOperation implements CycleEvent {
