package valius.model.minigames.inferno;
...
public class Tzkalzuk extends SingleInstancedArea {
	...
	public void tzkalzukSpecials(int tick) {
		...
		if(random == 0 && !singleRanger && !singleMager) {
			...
			World.getWorld().getNpcHandler().spawnNpc(player, InfernoWave.JAL_AKREK_XIL, player.getX(), player.getY() - 1, height, 0, 60, 8, 350, 540, true, false);
			World.getWorld().getNpcHandler().spawnNpc(player, InfernoWave.JAL_AKREK_KET, player.getX(), player.getY() - 1, height, 0, 60, 8, 350, 540, true, false);
			singleRanger = true;
			singleMager = true;
		}
			...
			if (TZKALZUK.getHealth().getAmount() < 480 && !singleJad) {
				...
				//World.getWorld().getNpcHandler().spawnNpc(player, REANIMATED_DEMON, player.absX + 1, player.absY, height, 0, 85, 8, 350, 540, true, false);
				//World.getWorld().getNpcHandler().spawnNpc(player, REANIMATED_DEMON, player.absX - 1, player.absY, height, 0, 85, 8, 350, 540, true, false);
				//World.getWorld().getNpcHandler().spawnNpc(player, REANIMATED_DEMON, player.absX, player.absY + 1, height, 0, 85, 8, 350, 540, true, false);
				singleJad = true;
			} else if (TZKALZUK.getHealth().getAmount() < 240) {
				//World.getWorld().getNpcHandler().spawnNpc(player, DARK_ANKOU, player.absX, player.absY - 1, height, 0, 60, 8, 350, 540, true, false);
	...
	public void spawnNpcs() {
		World.getWorld().getNpcHandler().spawnNpc(player, InfernoWave.TZKAL_ZUK, SPAWN_X, SPAWN_Y, height, 0, 1200, 251, 1000, 1000, true, false);
	}
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				if (cycle == 1) {
					...
					World.getWorld().getGlobalObjects().add(new GlobalObject(-1, 2267, 5368, (height + 1), 0, 10, -1, -1));  // Delete ceiling
					player.getPA().movePlayer(START_X, START_Y, height);
					World.getWorld().getNpcHandler().spawnNpc(player, InfernoWave.ANCESTRAL_GLYPH, GLYPH_SPAWN_X, GLYPH_SPAWN_Y, height, 0, 450, 38, 500, 700, false, false);
				} else if (cycle == 4) {
					World.getWorld().getGlobalObjects().add(new GlobalObject(30342, 2267, 5366, height, 1, 10, -1, -1));  // West Wall
					World.getWorld().getGlobalObjects().add(new GlobalObject(30341, 2275, 5366, height, 3, 10, -1, -1));  // East Wall
					World.getWorld().getGlobalObjects().add(new GlobalObject(30340, 2267, 5364, height, 1, 10, -1, -1));  // West Corner
					World.getWorld().getGlobalObjects().add(new GlobalObject(30339, 2275, 5364, height, 3, 10, -1, -1));  // East Corner
					World.getWorld().getGlobalObjects().add(new GlobalObject(30344, 2268, 5364, height, 3, 10, -1, -1)); // Set falling rocks - west
					World.getWorld().getGlobalObjects().add(new GlobalObject(30343, 2273, 5364, height, 3, 10, -1, -1)); // Set falling rocks - east
					World.getWorld().getGlobalObjects().add(new GlobalObject(-1, 2270, 5363, height, 3, 10, -1, -1)); // Delete ancestral glyph
