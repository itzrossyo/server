...
public class PotionMixing {
	...
	public static void decantInventory(Player player) {
		for (int index = 0; index < player.playerItems.length; index++) {
			...
			Item item = new Item(player.playerItems[index] - 1, player.playerItemsN[index], index);
			if (!ItemUtility.itemIsNote[item.getId()]) {
	...
	private static void decantUnnotedItem(Player player, Item item) {
		...
		while (hasChanged) {
			...
			hasChanged = false;
			item = new Item(player.playerItems[slot] - 1, player.playerItemsN[slot], slot);
			for (int index = player.playerItems.length - 1; index > slot; index--) {
				if (player.playerItems[index] - 1 <= 0) {
					continue;
				}
				Item item2 = new Item(player.playerItems[index] - 1, player.playerItemsN[index], index);
	...
	private static void decantNotedItem(Player player, Item item) {
		...
		while (totalDosage > 0 && dosage > 0) {
			...
			if (added > 0 && !player.getItems().addItem(potion.getItemId(dosage) + 1, added)) {
				World.getWorld().getItemHandler().createGroundItem(player, potion.getItemId(dosage) + 1, player.getX(), player.getY(), player.getHeight(), added);
			}
		...
		if (player.getItems().freeSlots() <= 0 && !player.getItems().playerHasItem(230)) {
			World.getWorld().getItemHandler().createGroundItem(player, 230, player.getX(), player.getY(), player.getHeight(), vialsToAdd);
		} else {
