...
public class AttackPlayer implements PacketType {
	...
	public void processPacket(Player player, int packetType, int packetSize) {
		...
		case ATTACK_PLAYER:
			...
			boolean usingCross = player.getItems().isWearingAnyItem(9185, 21902, 11785, 21012, 33114, 33117, 33094);
			...
			if ((player.getCombat().usingCrystalBow() || player.getItems().isWearingAnyItem(33030) || player.getItems().isWearingAnyItem(33908, 33909, 33910, 33911, 33914, 33922)) && player.usingArrows && player.getCombat().properBolts()) {
			...
			if (!player.getCombat().correctBowAndArrows() && Config.CORRECT_ARROWS && usingBow && !player.getCombat().usingCrystalBow()
					&& !player.getItems().isWearingAnyItem(4734, 9185, 21902, 11785, 21012, 33117, 33114, 19481, 19478, 33094)) {
			...
			if (player.getItems().isWearingAnyItem(9185, 21902, 11785, 21012, 33117, 33114, 33094) && !player.getCombat().properBolts()) {
			...
			if (player.getCombat().checkReqs()) {
				...
				if (!player.usingMagic && !usingBow && !usingOtherRangeWeapons) {
					...
					player.getMovementQueue().follow(PlayerHandler.players[player.playerIndex]);
