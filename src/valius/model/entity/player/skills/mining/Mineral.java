 ...
 */
public enum Mineral {
	CLAY(new int[] { 4437, 4438, 11363, 11362 }, "none", 1, 18, 0, 3, 15, 30000, true, generateExclusive(434)),
	COPPER(new int[] { 10943, 11161 }, "bronze", 1, 18, 5, 5, 15, 15000, true, generateExclusive(436)),
	TIN(new int[] { 11360, 11361 }, "bronze", 1, 18, 5, 5, 15, 15000, true, generateExclusive(438)),
	IRON(new int[] { 11365, 11364 }, "iron", 15, 35, 5, 8, 17, 14800, true, generateExclusive(440)),
	COAL(new int[] { 4676, 11366, 11367 }, "none", 30, 50, 3, 15, 29, 14600, true, generateExclusive(453)),
	SILVER(new int[] { 11368, 11369 }, "silver", 20, 40, 3, 15, 29, 14000, true, generateExclusive(442)),
	GOLD(new int[] { 11370, 11371 }, "gold", 40, 65, 3, 25, 32, 14200, true, generateExclusive(444)),
	MITHRIL(new int[] { 11372, 11373 }, "mithril", 55, 80, 3, 40, 35, 13800, true, generateExclusive(447)),
	ADAMANT(new int[] { 11374, 11375 }, "adamant", 70, 95, 2, 50, 37, 13200, true, generateExclusive(449)),
	RUNE(new int[] { 11376, 11377 }, "rune", 85, 125, 0, 100, 39, 12500, true, generateExclusive(451)),
	...
	/**
	 * An array of object ids that are associated with the mineral obtained from
	 * them
	 */
	 ...
	 * 
	 * @param objectIds            the objects that the mineral can be extracted
	 *                             from
	 * @param level                the level required to extract minerals from the
	 *                             object(s)
	 * @param experience           the experience gain after extraction
	 * @param depletionProbability the probability in terms of a 1:
	 *                             {@link #depletionProbability} ratio.
	 * @param respawnRate          the rate at which the mineral respawn's to the
	 *                             world
	 ...
	 * @param mineralReturn        the mineral that is returned to the player as a
	 *                             result of mining the resource
	 */
	private Mineral(int[] objectIds, String barName, int level, double experience, int depletionProbability,
			int respawnRate, int extractionRate, int petChance, boolean depletes, MineralReturn mineralReturn) {
	...
	/**
	 * Every mineral has a difference rate at which they deplete. Some minerals
	 * faster then others, and some minerals instantly after being extracted from.
	 * 
	 * @return the probability of depletion. When the probability is 0 the chance of
	 *         depletion is 1:1.
	 */
	...
	/**
	 * Accesses the {@link MineralReturn} instance's
	 * {@link MineralReturn#generate()} function.
	 * 
	...
	/**
	 * The identification value of the object with no mineral remaining after
	 * extraction
	 */
	public static final int EMPTY_VEIN = 2835;

	/**
	 * An unmodifiable set of {@link Mineral} objects that will be used as a
	 * constant for obtaining information about certain minerals.
	 */
	private static final Set<Mineral> MINERALS = Collections.unmodifiableSet(EnumSet.allOf(Mineral.class));

	/**
	 * Retrieves the {@link Mineral} object with the same objectId as the parameter
	 * passed.
	 * 
	...
	public static Mineral forObjectId(int objectId) {
		return MINERALS.stream().filter(mineral -> Arrays.stream(mineral.objectIds).anyMatch(id -> id == objectId))
