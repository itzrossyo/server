...
public class AgilityHandler {
				...
				public void stop() {
					if (c != null && !c.disconnected) {
						if (c.playerEquipment[c.playerWeapon] == -1) {
							...
							c.getCombat().getPlayerAnimIndex(ItemUtility.getItemName(c.playerEquipment[c.playerWeapon]).toLowerCase());
						}
					}
				}
			}, x2 + y2);
	...
	public void lapProgress(Player c, int progress, int obj) {
		if (System.currentTimeMillis() - lastClick < OBJECT_CLICK_DELAY) {
			//c.sendMessage("You must wait to do this.");
			return;
...
//		}
	...
	public boolean checkLevel(Player c, int objectId) {
		if (getLevelRequired(objectId) > c.getSkills().getLevel(Skill.AGILITY)) {
