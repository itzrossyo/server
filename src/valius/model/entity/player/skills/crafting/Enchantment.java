...
public class Enchantment {
	...
	public enum Enchant {
		...
		ZENYTE_RING(19538, 19550, 87, 97, 721, 452, 7),
		ZENYTE_NECKLACE(19535, 19547, 87, 97, 721, 452, 7),
		ZENYTE_AMULET(19541, 19553, 87, 97, 721, 452, 7),
	...
	private boolean hasRunes(Player player, int spellID) {
		...
		case 6003:
			...
			break;
		case 23649:
			if (!player.getCombat().checkMagicReqs(62)) {
				return false;
			}
			break;
		}
	...
	public void enchantItem(Player player, int itemID, int spellID) {
		...
		if (enc.equals(Enchant.DRAGONSTONE_RING)) {
			...
			player.sendMessage("of wealth can be bought through the voting, donation and pvm point store.");
			return;
		}
		if (player.getSkills().getLevel(Skill.MAGIC) >= enc.getLevelReq()) {
