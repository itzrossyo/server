...
public class Misc {
	...
	public static String to1Decimal(double input) {
	...
	public static PaletteTile[][] rotatePaletteClockwise(PaletteTile[][] array, int amt){
		if(amt > 0)
			return rotatePaletteClockwise(rotatePalette90Clockwise(array), amt - 1);
		else
			return array;
	}
	
	public static PaletteTile[][] rotatePalette90Clockwise(PaletteTile[][] original){
		PaletteTile[][] modified = new PaletteTile[2][2];
		modified[0][1] = original[1][0];
		modified[1][1] = original[0][0]; 
	...
	public static Item getItemFromList(List<Item> list) {
		if (list.isEmpty()) {
			return null;
		}
	...
	public static int toRandomCycle(int min, int max, TimeUnit unit) {
		return toCyclesOrDefault(random(min, max), min, unit);
	}
	...
	public static <T> T randomElementOf(T[] elements) {
		return elements[random(elements.length - 1)];
	}

	public static int randomElementOf(int[] elements) {
		return elements[random(elements.length - 1)];
	}
	...
	public static <T> List<T> shuffle(List<T> targets) {
		List<T> list = Lists.newArrayList(targets);
		Collections.shuffle(list);
		return list;
	}
	
	public static <T> List<T> randomSetOf(List<T> targets, long size){
		return shuffle(targets).stream().limit(size).collect(Collectors.toList());
	}
	
	public static String formatEnumName(Enum arg) {
		return arg.name().toLowerCase().replace("_", " ").toLowerCase();
	}

	public static String formatEnumName(String name) {
		return name.toLowerCase().replace("_", " ").substring(0).toUpperCase();
	}
	 ...
	 * @return
	 */
	public static String getDirectionFromInt(int directionOf) {
		switch (directionOf) {
