 ...
 * @author Divine
 */
public class ArmadylCasket implements CycleEvent {

    /**
     * The item id of the Casket required to trigger the event
     */
    private static final int ARMADYL_CASKET = 12024;
     ...
     */
    public ArmadylCasket(Player player) {
        this.player = player;
    }
    ...
    public void open() {
        ...
        if (player.getItems().freeSlots() < 2) {
            player.sendMessage("You need atleast two free slots to open a Armadyl Casket.");
            return;
        }
        if (!player.getItems().playerHasItem(ARMADYL_CASKET)) {
            player.sendMessage("You need a Armadyl Casket to do this.");
            return;
        }
        player.getItems().deleteItem(ARMADYL_CASKET, 1);
    ...
    public void execute(CycleEventContainer container) {
		...
		if (Misc.random(200) == 2 && player.getItems().getItemCount(19730, true) == 0 && player.summonId != 19730) {
			GlobalMessages.send(player.playerName + " has receive a Justicair Shield from a</col> @blu@Armadyl Casket<col/>@blu@!", GlobalMessages.MessageType.LOOT);
