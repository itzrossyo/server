...
public class DailyTasks {
	...
	public enum PossibleTasks {
		...
		LAW_RUNES(TaskTypes.SKILLING, 10, "craft Law runes 10 times."),
		...
		NATURE_RUNES(TaskTypes.SKILLING, 15, "Craft nature runes 10 times."),
		;
	...
	public static void assignTask(Player player) {
		if (player.dailyEnabled && player.playerChoice != null) {

			if(player.dailyTaskDate == getTodayDate() && player.currentTask != null) {
				...
				if (player.dailyTaskDate != getTodayDate() || player.dailyTaskDate == getTodayDate() && player.currentTask == null && !player.completedDailyTask) {
	...
	public static void complete(Player player) {
		...
		if(player.totalDailyDone >= player.currentTask.amount) {
			...
			player.sendMessage("@blu@@cr10@ You have completed your daily task: "+ Misc.capitalize(player.currentTask.name().toLowerCase().replace("_", " ")) + ", take these rewards!");
			player.getItems().addItemUnderAnyCircumstance(13307, 2000 + Misc.random(3000));
	...
	public static void increase(Player player, PossibleTasks task) {
		...
		if (player.currentTask.equals(task)) {
			...
			if(player.totalDailyDone % 5 == 0)
				player.sendMessage("<col=8d008d>" + player.totalDailyDone + "/"+player.currentTask.amount+" completed of your task.");
			complete(player);
		}
	}
